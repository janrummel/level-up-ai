...
```mermaid
graph LR
  subgraph HIER["You are HERE: Output API"]
    Schema["Zod Schema"] --> OutputObj["Output.object()"]
    OutputObj --> GT["generateText()"]
  end
  GT --> Typed["Typed Object"]

  subgraph Ohne["Without Schema"]
    Prompt2["Prompt"] --> GT2["generateText()"]
    GT2 --> Freitext["Unstructured Text"]
  end
...
```mermaid
graph LR
  Text["User Text"] --> Choice["Output.choice()<br/>(Category)"]
  Choice --> Cat["Category"]
  Cat --> Prompt2["Prompt + Category"]
  Prompt2 --> Obj["Output.object()<br/>(Details)"]
  Obj --> Result["Structured<br/>Result"]
