...
Especially important: If `finishReason` has the value `'length'`, the response was truncated. This means the token limit was reached before the LLM was done. You'll then see an incomplete response.

### Layer 3: The system + prompt combination

With the `system` parameter you give the LLM a role. The `prompt` is then the actual task:
