...
```mermaid
graph TD
  subgraph HIER["You are HERE: Model selection"]
    Aufgabe["Task"] --> Einfach{"Simple?"}
    Aufgabe --> Komplex{"Complex?"}
    Aufgabe --> Code{"Code?"}
    Einfach -->|Yes| Flash["Flash model<br/>cheap, fast<br/>gemini-2.5-flash"]
    Komplex -->|Yes| Pro["Pro model<br/>pricier, better<br/>claude-sonnet-4-5-20250514"]
    Code -->|Yes| CodeModel["Code model<br/>specialized<br/>e.g. claude-sonnet"]
  end
  ...
  style CodeModel fill:#E67E22,color:#fff
```
...
```mermaid
graph TD
  Task["Task"] --> Select["selectModel()"]
  Select --> Simple["Flash model"]
  Select --> Complex["Pro model"]
