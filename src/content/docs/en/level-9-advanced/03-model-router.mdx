...
<Image src={l9C2Overview} alt="Overview: User Input to Analyze Input, then to Flash Model (Simple), Pro Model (Complex), or Code Model (Code), all to Output" />
...
:::note[Prices as of March 2026]
Model prices change regularly — check the current prices on the respective provider pages ([Anthropic](https://docs.anthropic.com/en/docs/about-claude/models), [Google AI](https://ai.google.dev/gemini-api/docs/models)) before calculating costs.
:::
...
<Image src={l9C2Combine} alt="Combine: User Input to Model Router to Selected Model to Result to Usage Tracking to Cost Log" />
