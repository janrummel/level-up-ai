...
<Image src={overviewDiagram} alt="Frontend message is validated, if valid to generateText and save to DB, if invalid error response" />
...
Zod defines the expected structure of a message. You already know Zod from Level 3 (Tool Calling) — if not yet installed: `npm install zod`.
...
Create a file `challenge-4-4.ts`:
...
**Run with:** `npx tsx challenge-4-4.ts`
...
<Image src={combineDiagram} alt="Frontend is validated, if valid loadMessages, generateText, onFinish, saveMessages; if invalid 400 error" />
