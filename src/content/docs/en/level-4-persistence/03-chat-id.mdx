...
<Image src={overviewDiagram} alt="Frontend sends chatId to API handler, messages also go to API handler, generateText returns response" />
...
<Image src={combineDiagram} alt="Frontend sends chatId and messages to API handler, generateText, onFinish saves to chatStore by chatId" />
