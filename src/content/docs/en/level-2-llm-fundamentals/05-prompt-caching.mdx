...
|----------|---------|-------------------|----------------|
| Anthropic | Yes | 1,024 tokens (Sonnet), 2,048 (Haiku) | 5 minutes |
...
`.trim();
// This System Prompt has approx. 200+ words → well over 1024 tokens
...
The first request (Cache Write) is actually slightly more expensive than normal. But from the second request on, you save 90%. The break-even calculation:
...
**Explanation:** Both requests use the same System Prompt. On the second request the provider should read the System Prompt from cache. The `promptTokens` count stays the same (the tokens are still counted), but the effective costs are lower. Anthropic shows cache details in extended usage fields.
