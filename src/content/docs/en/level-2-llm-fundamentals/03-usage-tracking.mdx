...
```mermaid
graph LR
  subgraph HERE["You are HERE: Usage Tracking"]
...
Starting with AI SDK v6, some providers return extended token details. You'll find these in `result.usage` or in the `experimental_providerMetadata`:
...
### Layer 4: Session tracking with onFinish

For an application with multiple calls, you need a running total. The `onFinish` callback is perfect for this:
...
async function trackedGenerate(prompt: string) {
  ...
  const result = await generateText({
    ...
    prompt,
    onFinish({ usage }) {                                   // ← Callback for tracking
...
1. Import `selectModel` from Challenge 1.2 (or re-implement it)
2. Wrap the `generateText` call in a `trackedGenerate` function
3. Use `onFinish` to calculate and log costs after each call
