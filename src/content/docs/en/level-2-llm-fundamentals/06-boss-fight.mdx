...
graph LR
  ...
  Check -->|Fits| GT["generateText()"]
  Check -->|Too large| Truncate["Truncation"]
  Truncate --> GT
...
Update it in the `onFinish` callback or after each `generateText` call. The cache hit rate is `cacheHits / (cacheHits + cacheMisses) * 100`.
