...
<Image src={overviewDiagram} alt="Context window contains system prompt, messages, tool definitions and space for output; overflow causes error or truncation" />
...
<Image src={combineDiagram} alt="Messages and system prompt flow to context check, if OK to generateText, if too full to truncateMessages, then result.usage" />
