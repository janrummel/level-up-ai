...
<Image src={overviewDiagram} alt="New Input flows into a prompt with task-context, the highlighted examples tag (input/output pairs) and the-ask, which then goes to the LLM to produce consistent output in the desired style" />
...
<Image src={combineDiagram} alt="PromptConfig flows into buildSystemPrompt, Exemplars Array into a Prompt Template with examples slot, both feed into generateText to produce consistent output" />
