...
<Image src={overviewDiagram} alt="Left: vague prompt leads to unpredictable LLM output. Right (Basic Prompting): XML tags (task-context, rules, the-ask, output-format) produce a structured prompt that yields consistent, reproducible results" />
...
<Image src={combineDiagram} alt="PromptConfig flows into buildSystemPrompt, User Input into a Structured Prompt with XML Tags, both feed into generateText to produce consistent output" />
