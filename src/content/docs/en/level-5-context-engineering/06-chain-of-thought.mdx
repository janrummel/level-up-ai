...
graph LR
  ...
  subgraph HIER["You are HERE: Chain of Thought"]
    subgraph "With Chain of Thought"
      Q2["Question"] --> LLM2["LLM thinks<br/>step by step"]
      LLM2 --> TH["thinking-instructions<br/>+ output-format"]
      TH --> A2["Traceable<br/>Answer"]
    end
  end
...
```
Without CoT:  "What is 17 * 24?"  →  "408"  (guessed, wrong)
With CoT:     "What is 17 * 24?"  →  "17 * 20 = 340, 17 * 4 = 68, 340 + 68 = 408"  →  "408"
```
...
graph LR
  ...
  XML["XML Structure<br/>(5.2)"] --> BSP
  Exemplars["Exemplars<br/>(5.3)"] --> BSP
  Data["External Data<br/>(5.4)"] --> BSP
  CoT["Chain of Thought<br/>(5.5)"] --> BSP
  BSP --> Call["generateText()"]
  Call --> Output["Structured<br/>Answer"]
