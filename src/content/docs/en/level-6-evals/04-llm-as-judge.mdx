...
<Image src={overviewDiagram} alt="Input flows through task() to Output, which together with Expected goes to the Judge LLM, producing a Structured Score (A-E) that is converted to a Numeric Score (0-1)" />
...
<Image src={combineDiagram} alt="data (6.1) flows through task() to Output, which together with expected is evaluated by Contains Keyword (6.2) and Factuality (6.3), resulting in Scores in the Dashboard" />
