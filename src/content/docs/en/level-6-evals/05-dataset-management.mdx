...
<Image src={overviewDiagram} alt="Dataset management cycle: Seed Data collect, Evaluate run, Critique (assess data), Improve (add data) leads back to Evaluate — an iterative loop for improving test data quality" />
...
Create the file `dataset.eval.ts` and run it with `pnpm eval:dev`.
...
<Image src={combineDiagram} alt="Dataset (6.4) flows through task() to Output, which is evaluated by Deterministic (6.2) and Factuality (6.3), both feeding into the Dashboard" />
