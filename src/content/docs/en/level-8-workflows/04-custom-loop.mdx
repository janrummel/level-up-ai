...
What if you need more control than `ToolLoopAgent` provides — e.g., custom termination conditions, state tracking, or decisions between loop iterations?
...
**Without custom loop:** You use `ToolLoopAgent` or `generateText` with `stopWhen: stepCountIs(N)`. That's enough for standard cases. But: you can only break by step count, can't track your own state between iterations, and can't make decisions based on tool results.
