...
graph LR
  ...
  subgraph HIER["You are HERE: Agentic Loop"]
    LLM
    Check
    Exec
    Result
...
const calculatorTool = tool({
  ...
  execute: async ({ expression }) => ({
    expression,
    result: eval(expression),                            // ← Demo only! Never use eval in production.
  }),
});
...
The LLM might proceed like this: Step 1 → `search("Rust history")` → Step 2 → `calculator("2026 - 2010")` → Step 3 → `summarize(...)` → Step 4 → Final answer. The order is decided autonomously by the LLM.
