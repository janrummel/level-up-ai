...
<Image src={l7C3Overview} alt="Overview: LLM Provider Raw Stream passes through smoothStream() and Custom Filter Transform before arriving as UI Output" />
...
Create the file `stream-transforms.ts`:
...
Run: `npx tsx stream-transforms.ts`
...
```
STREAM TRANSFORMS ARE A MECHANISM THAT PROCESSES CHUNKS BETWEEN THE
LLM AND YOUR UI. THEY ALLOW YOU TO SMOOTH, FILTER, OR TRANSFORM
THE TEXT BEFORE IT REACHES THE USER.
```

> All text appears in uppercase and flows in word groups instead of individual characters. The exact text varies (LLM output).

</details>

## COMBINE

<Image src={l7C3Combine} alt="Combine: streamText() passes through smoothStream() and redactEmails() filter, Data Parts are added and sent to the Frontend" />
...
- [ai-hero-dev Exercises 07.01-07.04](https://github.com/ai-hero-dev/ai-sdk-v6-crash-course/tree/main/exercises/07-production-streaming) (Stream Transforms is based on official AI SDK documentation)
