...
Du baust einen **Token-Budget-Rechner** — ein Tool, das vor jedem LLM-Call prueft, ob der Request ins Budget passt. Der Rechner kombiniert alle vier Bausteine aus Level 2: Token-Counting, Kostenberechnung, Context-Window-Check und Cache-Tracking.
...
```mermaid
graph LR
  Input["System + User<br/>Prompt"] --> Count["Token<br/>Counting<br/>(2.1)"]
  Count --> Check["Context Window<br/>Check<br/>(2.3)"]
  Check -->|Passt| GT["generateText()"]
  Check -->|Zu gross| Truncate["Truncation"]
  Truncate --> GT
  GT --> Usage["Usage<br/>Tracking<br/>(2.2)"]
  Usage --> Cache["Cache<br/>Tracking<br/>(2.4)"]
  Cache --> Report["Session<br/>Report"]
...
4. **Cache-Tracking** (Challenge 2.4) — Tracke ueber mehrere Calls, ob der System Prompt gecacht wird. Berechne die Cache-Hit-Rate und die theoretische Ersparnis gegenueber "kein Caching".
...
//   - Berechne geschaetzte Tokens fuer System + User + Reserve
...
- [ ] Die Cache-Hit-Rate wird ueber die Session berechnet
- [ ] Ein finaler Session Report zeigt: Calls, Tokens, Kosten, Cache-Hit-Rate

## Hinweise

<details>
<summary>Hinweis 1: Token-Schaetzung verbessern</summary>

Die einfachste Variante ist `Math.ceil(text.length / 3.5)`. Fuer bessere Genauigkeit kannst Du unterschiedliche Faktoren fuer verschiedene Textarten verwenden: Code hat mehr Sonderzeichen (eigene Tokens), deutscher Text braucht mehr Tokens als englischer. Eine gewichtete Schaetzung koennte Woerter zaehlen und pro Wort 1.3 Tokens annehmen.
...
Ein einfaches Objekt reicht fuer das Session-Tracking:
...
Aktualisiere es nach jedem `generateText`-Call (bei `generateText` ist `result.usage` direkt nach dem `await` verfuegbar). Die Cache-Hit-Rate ist `cacheHits / (cacheHits + cacheMisses) * 100`.
