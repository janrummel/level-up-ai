...
import overviewDiagram from '../../../../assets/diagrams/l2-c3-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l2-c3-combine.svg';
...
<Image src={overviewDiagram} alt="Context Window enthaelt System Prompt, Messages, Tool-Definitionen und Platz fÃ¼r Output; bei Ueberschreitung Fehler oder Truncation" />

Das Context Window ist ein fester Rahmen — alles was das LLM gleichzeitig "sehen" kann. System Prompt, Chat-Verlauf, Tool-Definitionen UND der Platz fÃ¼r die Antwort mÃ¼ssen zusammen hineinpassen. Wenn nicht, gibt es Fehler oder Informationsverlust.

## WHY

**Ohne Context-Window-VerstÃ¤ndnis:** Dein Chat funktioniert fÃ¼r 10 Nachrichten, bei Nachricht 30 bekommst Du ploetzlich Fehler. Du schickst ein langes Dokument mit und die Antwort bezieht sich nur auf die letzten Absaetze. Du verstehst nicht, warum das LLM Kontext "vergisst".

**Mit Context-Window-VerstÃ¤ndnis:** Du planst strategisch, was ins Fenster kommt und was nicht. Du implementierst Truncation oder Summarization, bevor es zu Fehlern kommt. Du reservierst Platz fÃ¼r den Output und hast die volle Kontrolle Ã¼ber den Informationsfluss.

## WALKTHROUGH

### Schicht 1: Context-Window-Groessen im Ãberblick
...
> **Reasoning/Thinking Tokens:** Modelle wie Claude mit "Extended Thinking" oder OpenAI o3 nutzen zusaetzliche **Reasoning-Tokens**, um intern Schritt fÃ¼r Schritt zu denken, bevor sie antworten. Diese Tokens sind im Output nicht sichtbar, beanspruchen aber trotzdem das Context Window und zaehlen zu den Output-Tokens (= Kosten). Bei aktiviertem Extended Thinking kann ein einzelner Call tausende zusaetzliche Tokens verbrauchen. Das ist relevant fÃ¼r die Planung der verfÃ¼gbaren Kapazitaet — reserviere genuegend Platz fÃ¼r Reasoning, wenn Du diese Features nutzt.
...
const result = await generateText({
  ...
  // 3. Platz fÃ¼r Output — muss auch reinpassen!
  maxTokens: 4096,                                             // ← Reserviert fÃ¼r Antwort
});
```
...
> **`prompt` vs. `messages`:** In den bisherigen Challenges hast Du `prompt` genutzt — ein einzelner String fÃ¼r eine einzelne Frage. `messages` ist ein Array von Nachrichten mit `role` und `content`, das einen ganzen Chat-Verlauf abbildet. Beide Parameter schliessen sich gegenseitig aus: Nutze `prompt` fÃ¼r einzelne Anfragen und `messages` fÃ¼r Konversationen mit Verlauf.
...
Drei Strategien fÃ¼r lange Konversationen:

**Strategie 1: Truncation (Abschneiden aelterer Nachrichten)**
...
:::note[Model-Parameter: temperature und topP]
Neben dem Context Window gibt es weitere Parameter, die das LLM-Verhalten steuern:

**temperature** (0.0 – 1.0+): Kontrolliert die Zufaelligkeit der Antwort.
- `temperature: 0` — deterministisch, immer die wahrscheinlichste Antwort. Ideal fÃ¼r Code, Fakten, Analysen.
- `temperature: 1` — kreativer, mehr Variation. Gut fÃ¼r Brainstorming, kreatives Schreiben.
- Hoehe Werte (>1) erhÃ¶hen die Zufaelligkeit weiter, aber auch das Risiko von Unsinn.

**topP** (0.0 – 1.0): Begrenzt die Auswahl auf die wahrscheinlichsten Tokens, deren kumulative Wahrscheinlichkeit `topP` erreicht. `topP: 0.1` = nur die Top-10% der Tokens werden beruecksichtigt.

```typescript
const result = await generateText({
  model: anthropic('claude-sonnet-4-5-20250514'),
  temperature: 0,    // ← Deterministisch — ideal fÃ¼r Code-Reviews
  prompt: 'Review diesen Code: ...',
});
```

**Faustregel:** Nutze `temperature: 0` fÃ¼r Aufgaben die Praezision erfordern. Erhoehe temperature nur bewusst fÃ¼r kreative Aufgaben. In den meisten Faellen brauchst Du `topP` nicht zu Ã¤ndern.
:::

## TRY

**Aufgabe:** Simuliere eine Konversation, die sich dem Context Limit naehert. Berechne die Token-Anzahl und prÃ¼fe, ob sie ins Fenster passt.

Erstelle eine Datei `challenge-2-3.ts`:
...
- [ ] Token-Verbrauch fÃ¼r System, Messages und Output-Reserve berechnet
...
```
System: 45 Tokens
Messages: 6871 Tokens (10 Nachrichten)
Output-Reserve: 4096 Tokens
Gesamt: 11012 Tokens
Verbleibend: 188988 Tokens
Auslastung: 5.5%
```

Dieses Beispiel bleibt weit unter dem Limit. In einer echten Chat-Anwendung mit langen Code-Reviews und vielen Nachrichten wird die Auslastung schnell steigen.

**ErklÃ¤rung:** Die `estimateTokens`-Funktion nutzt die Faustregel 1 Token ≈ 3.5 Zeichen als Durchschnitt zwischen Deutsch und Englisch. In einer Produktionsanwendung wuerdest Du den Tokenizer des Providers verwenden fÃ¼r exakte Zahlen. Die 80%-Warnung gibt Dir einen Sicherheitspuffer, bevor es zu Fehlern kommt.

</details>

## COMBINE

<Image src={combineDiagram} alt="Messages und System Prompt fliessen in Context Check, bei OK zu generateText, bei zu voll zu truncateMessages, dann result.usage" />

**Uebung:** Baue einen Context Manager, der vor jedem Call prÃ¼ft, ob der Input ins Context Window passt, und bei Ueberschreitung automatisch alte Messages entfernt.
...
5. Erst dann den `generateText`-Call ausfÃ¼hren
