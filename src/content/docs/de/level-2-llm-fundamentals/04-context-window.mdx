...
> **Reasoning/Thinking Tokens:** Modelle wie Claude mit "Extended Thinking" oder OpenAI o3 nutzen zusaetzliche **Reasoning-Tokens**, um intern Schritt fuer Schritt zu denken, bevor sie antworten. Diese Tokens sind im Output nicht sichtbar, beanspruchen aber trotzdem das Context Window und zaehlen zu den Output-Tokens (= Kosten). Bei aktiviertem Extended Thinking kann ein einzelner Call tausende zusaetzliche Tokens verbrauchen. Das ist relevant fuer die Planung der verfuegbaren Kapazitaet — reserviere genuegend Platz fuer Reasoning, wenn Du diese Features nutzt.
...
> **`prompt` vs. `messages`:** In den bisherigen Challenges hast Du `prompt` genutzt — ein einzelner String fuer eine einzelne Frage. `messages` ist ein Array von Nachrichten mit `role` und `content`, das einen ganzen Chat-Verlauf abbildet. Beide Parameter schliessen sich gegenseitig aus: Nutze `prompt` fuer einzelne Anfragen und `messages` fuer Konversationen mit Verlauf.
...
- `temperature: 0` — deterministisch, immer die wahrscheinlichste Antwort. Ideal fuer Code, Fakten, Analysen.
- `temperature: 1` — kreativer, mehr Variation. Gut fuer Brainstorming, kreatives Schreiben.
- Hoehe Werte (>1) erhoehen die Zufaelligkeit weiter, aber auch das Risiko von Unsinn.
...
const result = await generateText({
  model: anthropic('claude-sonnet-4-5-20250514'),
  temperature: 0,    // ← Deterministisch — ideal fuer Code-Reviews
  prompt: 'Review diesen Code: ...',
});
```

**Faustregel:** Nutze `temperature: 0` fuer Aufgaben die Praezision erfordern. Erhoehe temperature nur bewusst fuer kreative Aufgaben. In den meisten Faellen brauchst Du `topP` nicht zu aendern.
:::
