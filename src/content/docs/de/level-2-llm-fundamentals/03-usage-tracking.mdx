...
<Image src={overviewDiagram} alt="generateText gibt result mit usage-Objekt zurueck: promptTokens, completionTokens, totalTokens fliessen in Kostenberechnung" />
...
### Schicht 4: Session-Tracking ueber mehrere Calls

Fuer eine Anwendung mit mehreren Calls brauchst Du einen Gesamtzaehler. Bei `generateText` ist das Ergebnis nach dem `await` direkt verfuegbar — Du greifst einfach auf `result.usage` zu:
...
> **Hinweis:** Bei `streamText` gibt es einen `onFinish`-Callback, der aufgerufen wird wenn der Stream endet. Bei `generateText` brauchst Du keinen Callback — das Ergebnis ist nach dem `await` sofort verfuegbar.
...
<Image src={combineDiagram} alt="Aufgabe fliesst in selectModel zum Modell, generateText gibt result.usage zurueck, calculateCost berechnet Kosten-Log" />
