...
|----------|---------|-------------------|-------------|
| Anthropic | Ja | 1.024 Tokens (Sonnet), 2.048 (Haiku) | 5 Minuten |
...
Der erste Request (Cache Write) ist sogar etwas teurer als normal. Aber ab dem zweiten Request sparst Du 90%. Die Break-Even-Rechnung:
...
**Erklaerung:** Beide Requests nutzen denselben System Prompt. Beim zweiten Request sollte der Provider den System Prompt aus dem Cache lesen. Die `promptTokens`-Zahl bleibt gleich (die Tokens werden trotzdem gezaehlt), aber die effektiven Kosten sind niedriger. Anthropic zeigt Cache-Details in erweiterten Usage-Feldern an.
