...
description: "Wie Dein Backend weiss, zu welchem Chat eine neue Nachricht gehÃ¶rt — Session-Management mit Chat IDs."
sidebar:
  order: 3
---

import { Image } from 'astro:assets';
import overviewDiagram from '../../../../assets/diagrams/l4-c2-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l4-c2-combine.svg';

## THINK

Wie weiss Dein Backend, zu welchem Chat eine neue Nachricht gehÃ¶rt?

## OVERVIEW

<Image src={overviewDiagram} alt="Frontend sendet chatId an API Handler, messages gehen auch an API Handler, generateText gibt Response zurÃ¼ck" />
...
**Ohne Chat ID:** Jede Nachricht ist fÃ¼r Dein Backend ein neuer, anonymer Request. Es gibt keinen Zusammenhang zwischen "Hallo" und der Folgefrage "Was meinst Du damit?". Du kannst keinen Verlauf laden, keine Session fortsetzen, kein Chat-Management bauen.

**Mit Chat ID:** Jede Nachricht gehÃ¶rt zu einem identifizierbaren Chat. Du kannst den Verlauf laden, neue Messages anhaengen, mehrere parallele Chats verwalten und dem User eine Chat-History anbieten.
...
`crypto.randomUUID()` ist in Node.js (ab v19) und allen modernen Browsern verfÃ¼gbar. Die Wahrscheinlichkeit einer Kollision ist praktisch null (2^122 moegliche Werte).
...
Die Chat ID bleibt fÃ¼r die gesamte Konversation gleich. Neue Nachrichten werden mit derselben ID geschickt. Ein neuer Chat bekommt eine neue ID.

### Schicht 3: Chat ID im Backend lesen

Das Backend extrahiert die Chat ID aus dem Request und nutzt sie fÃ¼r die Zuordnung:
...
Erstelle eine Datei `challenge-4-2.ts`:
...
**Ausfuehren mit:** `npx tsx challenge-4-2.ts`
...
**Erwarteter Output (Chat IDs variieren):**
...
**ErklÃ¤rung:** `crypto.randomUUID()` generiert eindeutige IDs. Die `Map` speichert Messages gruppiert nach Chat ID. Durch das Kuerzen der ID im Log bleibt die Ausgabe lesbar. Chat A hat zwei Messages, Chat B eine — sauber getrennt.

</details>

## COMBINE

<Image src={combineDiagram} alt="Frontend sendet chatId und messages an API Handler, generateText, onFinish speichert in chatStore nach chatId" />
