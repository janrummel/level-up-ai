...
import overviewDiagram from '../../../../assets/diagrams/l4-c4-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l4-c4-combine.svg';
...
<Image src={overviewDiagram} alt="Frontend Message wird validiert, bei valid zu generateText und Save to DB, bei invalid Error Response" />
...
### Schicht 1: Zod Schema fÃ¼r Messages
...
// Option 2: safeParse — gibt Result-Objekt zurÃ¼ck (kein throw)
...
`safeParse` ist fÃ¼r API-Handler die bessere Wahl: Du bekommst strukturierte Fehler zurÃ¼ck und kannst dem Client eine sinnvolle Fehlermeldung schicken, statt einen unbehandelten Error zu werfen.
...
```typescript
// ❌ ANTI-PATTERN: String-Matching fÃ¼r Injection-Schutz
...
- **Strukturelle Validation** (Schicht 1-3 oben): Laenge, Typ, Format prÃ¼fen
...
- [ ] Gueltiger Input gibt `{ success: true, data: ... }` zurÃ¼ck
- [ ] Ungueltiger Input gibt lesbaren Error-String zurÃ¼ck
...
**ErklÃ¤rung:** `safeParse` gibt ein Result-Objekt zurÃ¼ck statt einen Error zu werfen. Die `error.issues` enthalten den Pfad zum fehlerhaften Feld und eine Beschreibung. Die Custom Error Messages (z.B. "Content darf nicht leer sein") machen die Fehlermeldungen fÃ¼r Entwickler und User verstÃ¤ndlich.

</details>

## COMBINE

<Image src={combineDiagram} alt="Frontend wird validiert, bei valid loadMessages, generateText, onFinish, saveMessages; bei invalid 400 Error" />
