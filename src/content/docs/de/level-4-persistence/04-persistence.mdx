...
Persistence bedeutet: Messages werden nach jedem LLM-Call gespeichert und beim nÃ¤chsten Request (oder nach einem Reload) wieder geladen. Der Chat "erinnert sich" an alles, was vorher gesagt wurde.
...
**Mit Persistence:** Der Verlauf ueberlebt Reloads, Server-Neustarts und Deployments. Der User kann einen Chat jederzeit fortsetzen. Du hast eine Grundlage fÃ¼r Chat-History, Suche und Analytics.

## WALKTHROUGH

### Schicht 1: Der Persistence-Cycle

Der vollstÃ¤ndige Cycle besteht aus drei Schritten:
...
Beim nÃ¤chsten Request (oder nach Reload) laedt das Backend den bisherigen Verlauf:
...
Das LLM bekommt bei jedem Call den gesamten Verlauf und kann auf fruehere Nachrichten Bezug nehmen. Ohne Persistence wÃ¼rde es die zweite Frage ohne Kontext beantworten.
...
Zwei getrennte Tabellen: `chats` fÃ¼r Metadaten, `messages` fÃ¼r den Inhalt. Verknuepft Ã¼ber `chatId`. Das ermoeglicht effiziente Queries wie "alle Chats des Users" oder "Token-Verbrauch pro Chat".
...
**Erwarteter Output (ungefÃ¤hr — LLM-Antworten variieren):**
...
**ErklÃ¤rung:** `continueChat` implementiert den vollstaendigen Persistence-Cycle: Load → Generate → Save. Das LLM bekommt bei jedem Call den gesamten Verlauf und kann auf fruehere Nachrichten Bezug nehmen. Die `Map` simuliert eine Datenbank — in Production wÃ¼rde hier ein SQL INSERT / SELECT stehen.
...
3. `load(chatId)` gibt den Verlauf zurÃ¼ck (leeres Array als Fallback)
4. `list()` gibt alle Chat IDs mit Message-Anzahl zurÃ¼ck
5. Baue eine `chat(db, chatId, userMessage)`-Funktion die den Cycle ausfÃ¼hrt
