...
`generateText` ruft nach der vollstaendigen Completion einen `onFinish` Callback auf. In diesem Callback hast Du Zugriff auf `text`, `usage`, `finishReason` und die vollstÃ¤ndige `response` — der perfekte Ort, um Ergebnisse zu speichern, Tokens zu tracken oder Logs zu schreiben.
...
**Mit `onFinish`:** Das Speichern passiert automatisch, direkt nachdem das LLM fertig ist. Der Callback wird nach erfolgreicher Completion aufgerufen. Du hast einen zentralen Ort fÃ¼r Logging, Analytics und Persistence.
...
Der Callback bekommt ein Objekt mit den wichtigsten Properties (die vollstÃ¤ndige Liste findest Du in der [generateText API Reference](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-text)):
...
**Token-Tracking fÃ¼r Kostenberechnung:**
...
Bei `streamText` feuert `onFinish` erst, wenn der letzte Chunk gestreamt wurde. Der vollstÃ¤ndige `text` ist dann im Callback verfÃ¼gbar.
...
**ErklÃ¤rung:** Der `onFinish` Callback wird automatisch nach der Completion aufgerufen. Er loggt die Token-Nutzung und den Finish Reason und speichert einen Audit-Eintrag. Das `auditLog` ist nach dem `await` befuellt, weil `onFinish` vor dem Resolve laeuft.
...
1. Erstelle eine Variable `totalCost` die Ã¼ber mehrere Calls hinweg addiert
...
**Optional Stretch Goal:** Baue eine `withCostTracking`-Wrapper-Funktion, die `generateText`-Optionen entgegennimmt und automatisch einen `onFinish` Callback fÃ¼r Cost-Tracking injiziert.
