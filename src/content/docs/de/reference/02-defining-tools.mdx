...
| `parameters` | `ZodSchema \| JSONSchema` | Schema der Eingabeparameter — Zod empfohlen fuer Typsicherheit |
...
| Property | Typ | Beschreibung |
|----------|-----|-------------|
| `experimental_toToolResultContent` | `(result) => Content[]` | Wandelt das Tool-Ergebnis in Content um, den das LLM verarbeiten kann — z.B. fuer Bilder oder Screenshots bei Multi-Modal-Modellen |

Die `execute`-Funktion erhaelt neben den Argumenten ein Options-Objekt mit:
- `toolCallId` — eindeutige ID des Tool-Calls (z.B. fuer Stream-Data)
...
`needsApproval` (boolean oder Funktion) aktiviert das Tool-Approval-Pattern — siehe [Tool Approval](/de/level-3-agents/06-tool-approval). Fuer `outputSchema` und `title` pruefe die [offizielle tool()-Referenz](https://ai-sdk.dev/docs/reference/ai-sdk-core/tool).
:::
