...
`streamText` gibt ein Result-Objekt zurÃ¼ck, das zwei AsyncIterables bereitstellt: `textStream` liefert nur Text-Chunks als Strings — ideal fÃ¼r einfache Ausgaben. `fullStream` liefert alle Stream Part Types inklusive Tool-Calls, Reasoning, Sources und Step-Grenzen.
...
| `finishReason` | `Promise<FinishReason>` | Grund fÃ¼r das Stoppen |
