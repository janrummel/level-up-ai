...
> **Wichtig:** Jeder `generateText`/`streamText` Call ist **zustandslos** — das LLM erinnert sich nicht an vorherige Nachrichten. Wenn Du einen Chat bauen willst, musst Du den gesamten Nachrichtenverlauf bei jedem Call als `messages` Array mitschicken. Ohne das antwortet das LLM auf jede Nachricht, als wÃ¤re es die erste.

Dieses Projekt verbindet alle sechs Bausteine:

<Image src={overviewDiagram} alt="User Input fliesst in selectModel zum Modell, zusammen mit System Prompt in streamText/Output.object zum CLI Output" />
...
3. **streamText fÃ¼r normale Antworten** (Challenge 1.4) — Normale Nachrichten werden mit `streamText` beantwortet. Text erscheint Token fÃ¼r Token im Terminal.
...
// fÃ¼r kurze und anthropic('claude-sonnet-4-5-20250514') fÃ¼r lange Nachrichten.

// TODO: Definiere einen System Prompt mit Rolle und Regeln

// TODO: Definiere ein Zod Schema fÃ¼r den /json Modus
...
console.log('Starte eine Nachricht mit /json fÃ¼r strukturierte Ausgabe.\n');
askQuestion();
```
...
- [ ] Normale Nachrichten werden mit `streamText` Token fÃ¼r Token ausgegeben
...
FÃ¼r `streamText` nutze `process.stdout.write(chunk)` statt `console.log(chunk)`, damit der Text ohne Zeilenumbrueche Wort fÃ¼r Wort erscheint. Nach dem Stream einen `console.log()` fÃ¼r den finalen Zeilenumbruch. Den Token-Verbrauch bekommst Du aus `fullStream` (Event-Typ `finish`) oder Ã¼ber `result.usage` (ein Promise, das Du nach dem Stream awaiten kannst).
...
Pruefe mit `message.startsWith('/json')` ob der JSON-Modus aktiviert werden soll. Der eigentliche Prompt ist dann `message.slice(5).trim()` — alles nach "/json". FÃ¼r den JSON-Modus nutze `generateText` (nicht `streamText`), weil `Output.object` ein vollstaendiges Objekt braucht, kein Stream.
