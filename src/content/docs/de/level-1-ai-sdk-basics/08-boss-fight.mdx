...
> **Wichtig:** Jeder `generateText`/`streamText` Call ist **zustandslos** — das LLM erinnert sich nicht an vorherige Nachrichten. Wenn Du einen Chat bauen willst, musst Du den gesamten Nachrichtenverlauf bei jedem Call als `messages` Array mitschicken. Ohne das antwortet das LLM auf jede Nachricht, als waere es die erste.
