...
Wenn Du mit Claude, GPT und Gemini arbeiten willst — brauchst Du fÃ¼r jeden eine andere Library? Andere Typen? Andere Fehlerbehandlung?

## OVERVIEW

<Image src={overviewDiagram} alt="AI SDK Architektur: Dein Code verbindet sich Ã¼ber AI SDK Core, UI und RSC mit Providern wie Anthropic, OpenAI und Google" />
...
**Ohne AI SDK:** FÃ¼r jeden Provider eine andere API, andere Typen, andere Fehlerbehandlung. Provider wechseln bedeutet den halben Code umschreiben. Jeder Provider hat eigene SDKs mit unterschiedlichen Interfaces.

**Mit AI SDK:** Eine API fÃ¼r alle Provider. Du wechselst den Provider, indem Du eine Import-Zeile und den Modell-Namen aenderst. Der Rest Deines Codes bleibt identisch.
...
| **AI SDK UI** | `ai` (gleicher Import) | Frontend: Framework-agnostische Hooks fÃ¼r Chat-UIs |
...
Drei Zeilen Code, drei Informationen zurÃ¼ck:
1. **`result.text`** — der generierte Text als String
2. **`result.usage`** — wie viele Tokens verbraucht wurden (wichtig fÃ¼r Kosten)
3. **`result.finishReason`** — warum das LLM aufgehoert hat zu generieren

Das `model`-Argument ist der einzige Ort, an dem der Provider vorkommt. Alles andere (`prompt`, `result.text`, `result.usage`) ist Provider-unabhÃ¤ngig.
...
**ErklÃ¤rung:** `generateText` ist eine async Funktion, die ein Result-Objekt zurueckgibt. Das `model`-Argument erstellt eine Anthropic-Modell-Instanz mit dem Modell-Namen `claude-sonnet-4-5-20250514`. Der `prompt` ist der User-Input. Die drei geloggten Properties geben Dir den generierten Text, den Token-Verbrauch und den Grund fÃ¼r das Stoppen.
...
**Uebung:** Aendere den Provider — ersetze `anthropic` durch `openai`. Was musst Du Ã¤ndern?
