...
```mermaid
graph LR
  subgraph HIER["Du bist HIER: System Prompt"]
    System["system<br/>(Rolle, Regeln, Stil)"]
  end
  User["prompt<br/>(Aufgabe)"] --> LLM["LLM"]
  System --> LLM
  LLM --> Antwort["Antwort im<br/>definierten Stil"]
...
System Prompts sind Verhaltensanweisungen, keine Sicherheitsmechanismen. Fuer echten Schutz brauchst Du separate Input-Validation und Guardrails — das lernst Du in [Level 9: Guardrails](/de/level-9-advanced/02-guardrails/).
:::
...
```mermaid
graph LR
  System["System Prompt<br/>(Rolle: Code Reviewer)"] --> GT["generateText()"]
  Prompt["User Input"] --> GT
  GT --> Output["Output.object()<br/>(Strukturiertes Feedback)"]
