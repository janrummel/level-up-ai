...
Wie bringst Du ein LLM dazu, JSON statt Freitext zurueckzugeben — zuverlÃ¤ssig, jedes Mal? Und wie stellst Du sicher, dass das JSON genau die Felder und Typen hat, die Du brauchst?
...
> Falls noch nicht installiert: `npm install zod`. Zod ist eine TypeScript-first Schema-Validierungs-Library — Du definierst die erwartete Struktur, und Zod sorgt fÃ¼r Typensicherheit zur Compile- und Laufzeit.
...
const { output } = await generateText({
  ...
  prompt: 'Erstelle ein Rezept fÃ¼r Pasta Carbonara.',
});
...
FÃ¼r Klassifikation oder einfache Entscheidungen gibt es `Output.choice`:
...
`Output.choice` ist ideal fÃ¼r Routing-Entscheidungen: Das LLM waehlt eine Option, und Du reagierst basierend auf der Wahl. Kein Freitext-Parsing, keine unsicheren `includes()`-Checks.
...
// TODO 1: Definiere ein Zod Schema fÃ¼r eine Produktbewertung
...
- [ ] `z.describe()` fÃ¼r mindestens ein Feld genutzt
...
**ErklÃ¤rung:** Das Zod Schema definiert exakt, welche Felder und Typen Du erwartest. `Output.object` sorgt dafuer, dass das LLM ein valides Objekt generiert, das diesem Schema entspricht. `z.describe()` gibt dem LLM zusaetzlichen Kontext — besonders bei `rating` hilft es, den erwarteten Wertebereich zu kommunizieren.
...
Nachteile: [ 'Hoher Preis', 'Wenig Design-Aenderung zum Vorgaenger' ]
...
**Optional Stretch Goal:** Nutze die `selectModel`-Funktion aus Challenge 1.2, um fÃ¼r den Klassifikations-Call ein guenstiges Flash-Modell und fÃ¼r die Detail-Extraktion ein Pro-Modell zu waehlen.
