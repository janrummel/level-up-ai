...
description: "Lerne, wie Du mit streamText Echtzeit-Feedback erzeugst — Token für Token statt alles auf einmal."
sidebar:
  order: 5
---
...
<Image src={overviewDiagram} alt="streamText sendet Prompt an LLM, LLM streamt text-delta Chunks zurück an den Code" />

Das LLM generiert Token für Token. Mit `streamText` bekommst Du jeden Token sofort, statt auf die komplette Antwort zu warten. Die gestrichelten Pfeile zeigen: Daten fliessen stueckweise zurück.
...
`streamText` funktioniert wie `generateText` — selbe Parameter, selbes Interface. Der Unterschied: Es gibt sofort ein Stream-Objekt zurück, statt auf die komplette Antwort zu warten.
...
Wichtig: `streamText` wird **nicht** mit `await` aufgerufen. Die Funktion gibt synchron ein Result-Objekt zurück, das mehrere Streams enthaelt. Das eigentliche Warten passiert beim Konsumieren der Streams.
...
Warum `process.stdout.write` statt `console.log`? Weil `console.log` nach jedem Aufruf einen Zeilenumbruch einfuegt. `process.stdout.write` schreibt den Text genau so, wie er ankommt — Wort für Wort in derselben Zeile.
...
> **`finishReason`:** Die moeglichen Werte sind `stop` (Modell ist fertig), `length` (max. Tokens erreicht), `content-filter`, `tool-calls`, `error` und `other`. Die vollständige Liste findest du in der [AI SDK Dokumentation](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-text#finish-reason).

### Schicht 4: toUIMessageStreamResponse — Vorschau auf spaetere Levels

Für Web-APIs (z.B. Next.js) gibt es `toUIMessageStreamResponse()`. Damit wird der Stream direkt in eine HTTP Response umgewandelt:
...
- [ ] Text wird Chunk für Chunk ins Terminal geschrieben (mit `process.stdout.write`)
...
**Erklärung:** `streamText` gibt synchron ein Result-Objekt zurück. Der eigentliche API-Call startet erst, wenn Du den Stream konsumierst (mit `for await`). `textStream` liefert nur Text-Chunks, `fullStream` liefert alle Event-Typen inklusive Metadaten wie Token-Verbrauch.
...
(Text erscheint Wort für Wort im Terminal)
