...
description: "Lerne, wie Du mit streamText Echtzeit-Feedback erzeugst — Token fÃ¼r Token statt alles auf einmal."
sidebar:
  order: 5
---
import { Image } from 'astro:assets';
import overviewDiagram from '../../../../assets/diagrams/l1-c4-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l1-c4-combine.svg';

## THINK

Warum siehst Du in ChatGPT die Antwort Wort fÃ¼r Wort statt alles auf einmal? Und was passiert unter der Haube, damit das funktioniert?

## OVERVIEW

<Image src={overviewDiagram} alt="streamText sendet Prompt an LLM, LLM streamt text-delta Chunks zurÃ¼ck an den Code" />

Das LLM generiert Token fÃ¼r Token. Mit `streamText` bekommst Du jeden Token sofort, statt auf die komplette Antwort zu warten. Die gestrichelten Pfeile zeigen: Daten fliessen stueckweise zurÃ¼ck.
...
`streamText` funktioniert wie `generateText` — selbe Parameter, selbes Interface. Der Unterschied: Es gibt sofort ein Stream-Objekt zurÃ¼ck, statt auf die komplette Antwort zu warten.
...
Wichtig: `streamText` wird **nicht** mit `await` aufgerufen. Die Funktion gibt synchron ein Result-Objekt zurÃ¼ck, das mehrere Streams enthaelt. Das eigentliche Warten passiert beim Konsumieren der Streams.

### Schicht 2: textStream — AsyncIterable fÃ¼r Text-Chunks
...
Warum `process.stdout.write` statt `console.log`? Weil `console.log` nach jedem Aufruf einen Zeilenumbruch einfuegt. `process.stdout.write` schreibt den Text genau so, wie er ankommt — Wort fÃ¼r Wort in derselben Zeile.
...
> **`finishReason`:** Die moeglichen Werte sind `stop` (Modell ist fertig), `length` (max. Tokens erreicht), `content-filter`, `tool-calls`, `error` und `other`. Die vollstÃ¤ndige Liste findest du in der [AI SDK Dokumentation](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-text#finish-reason).

### Schicht 4: toUIMessageStreamResponse — Vorschau auf spaetere Levels

FÃ¼r Web-APIs (z.B. Next.js) gibt es `toUIMessageStreamResponse()`. Damit wird der Stream direkt in eine HTTP Response umgewandelt:
...
- [ ] Text wird Chunk fÃ¼r Chunk ins Terminal geschrieben (mit `process.stdout.write`)
...
**ErklÃ¤rung:** `streamText` gibt synchron ein Result-Objekt zurÃ¼ck. Der eigentliche API-Call startet erst, wenn Du den Stream konsumierst (mit `for await`). `textStream` liefert nur Text-Chunks, `fullStream` liefert alle Event-Typen inklusive Metadaten wie Token-Verbrauch.

> **Tipp:** Alternativ zu `fullStream` kannst Du nach dem Stream auch `await result.usage` nutzen — das Result-Objekt hat `usage` als Promise, das aufloest sobald der Stream fertig ist.
...
```
TypeScript ist besser als JavaScript weil es statische Typen...
(Text erscheint Wort fÃ¼r Wort im Terminal)
...
<Image src={combineDiagram} alt="Aufgabe fliesst in selectModel, waehlt Modell A und B, beide durch streamText zu Stream A und Stream B" />
