...
> **`finishReason`:** Die moeglichen Werte sind `stop` (Modell ist fertig), `length` (max. Tokens erreicht), `content-filter`, `tool-calls`, `error` und `other`. Die vollstaendige Liste findest du in der [AI SDK Dokumentation](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-text#finish-reason).
...
(Text erscheint Wort fuer Wort im Terminal)
