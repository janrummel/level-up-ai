---
title: "Boss Fight: Eval-Pipeline fÃ¼r Chat Titles"
description: "Kombiniere alle fuenf Bausteine aus Level 6 in einer kompletten Eval-Pipeline fÃ¼r einen Chat-Titel-Generator."
sidebar:
  order: 7
  badge:
    text: 'Boss Fight'
...
Du baust eine **komplette Eval-Pipeline fÃ¼r einen Chat-Titel-Generator**. Das System nimmt eine User-Nachricht und generiert einen kurzen, beschreibenden Titel fÃ¼r den Chat. Deine Pipeline evaluiert die QualitÃ¤t dieser Titel automatisch — mit deterministischen Scorern UND LLM-as-Judge.
...
Input:  "Wie konfiguriere ich Docker Compose fÃ¼r ein Multi-Container Setup?"
...
5. **Deterministic Scorer: Kein Punkt am Ende** (Challenge 6.2) — Erstelle einen Scorer, der prÃ¼ft, ob der Titel NICHT mit einem Punkt endet.
...
7. **Mehrere Scorer kombiniert** — Alle drei Scorer (Titellaenge, kein Punkt, Relevanz) mÃ¼ssen in einem `scorers`-Array laufen.
...
//   - Gib score + metadata (rationale) zurÃ¼ck

// TODO 4: Definiere den System Prompt fÃ¼r den Titel-Generator
...
- [ ] System Prompt definiert Regeln fÃ¼r Titel-Generierung (Laenge, Format, Stil)
- [ ] `titleLength` Scorer bewertet die Laenge mit abgestuften Scores
- [ ] `noTrailingPeriod` Scorer prÃ¼ft auf fehlendes Punkt am Ende
...
Ein guter System Prompt fÃ¼r Titel-Generierung kÃ¶nnte so aussehen:
...
Der Judge-Prompt fÃ¼r Relevanz kÃ¶nnte die Frage stellen: "How well does this title describe the chat message?" und eine Skala anbieten:
