...
description: "Baue repraesentative, diverse Test-Datensaetze und lerne, wie Du die Qualität Deiner Eval-Daten systematisch pruefst."
sidebar:
  order: 5
---
...
<Image src={overviewDiagram} alt="Dataset-Management-Zyklus: Seed Data sammeln, Evaluate ausführen, Critique (Daten bewerten), Improve (Daten ergaenzen) fuehrt zurück zu Evaluate — ein iterativer Kreislauf zur Verbesserung der Test-Daten" />

Dataset Management ist ein Zyklus: Du sammelst Seed-Daten, fuehrst Evals aus, bewertest die Qualität Deiner Test-Daten, ergaenzt fehlende Faelle und evaluierst erneut. Die Daten werden mit jedem Durchlauf besser.
...
evalite('Chat Titles', {
  data: async () => {
    ...
    return [
      ...
      {
        input: 'Wie konfiguriere ich ESLint für ein React-Projekt?',
...
| Phase | Anzahl | Zweck |
|-------|--------|-------|
| **Prototyp** | 5-10 | Schnelles Feedback, grundlegende Funktion prüfen |
...
data: async () => [
  ...
  { input: 'Was ist O(n log n)?', expected: 'Algorithmus-Komplexität' },
...
Definiere Kategorien für Dein Dataset und stelle sicher, dass jede abgedeckt ist:

```typescript
// Dataset-Kategorien für einen Chat-Title-Generator
const categories = {
  technical: [
...
Eine fortgeschrittene Technik: Lass ein LLM die Qualität Deiner Test-Daten bewerten. "Sind meine Test-Cases repraesentativ genug? Welche Faelle fehlen?"
...
Das Ergebnis sagt Dir, welche Kategorien fehlen und schlägt konkrete Test-Cases vor. Du pruefst die Vorschlaege, uebernimmst die sinnvollen und fuehrst die Evals erneut aus.

### Schicht 5: Dataset versionieren

Dein Dataset ändert sich über die Zeit. Versioniere es wie Code:
...
So kannst Du später vergleichen: Hat sich der Score mit Dataset v2 verändert?

## TRY

**Aufgabe:** Erstelle ein Dataset für einen Chat-Title-Generator mit mindestens 15 Test-Cases aus verschiedenen Kategorien.
...
const chatTitleDataset = [
  ...
  { input: 'Ich verstehe das nicht, kannst du es nochmal erklären?', expected: 'Erklärung wiederholen' },
