...
description: "Baue repraesentative, diverse Test-Datensaetze und lerne, wie Du die QualitÃ¤t Deiner Eval-Daten systematisch pruefst."
sidebar:
  order: 5
---
import { Image } from 'astro:assets';
import overviewDiagram from '../../../../assets/diagrams/l6-c4-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l6-c4-combine.svg';
...
<Image src={overviewDiagram} alt="Dataset-Management-Zyklus: Seed Data sammeln, Evaluate ausfÃ¼hren, Critique (Daten bewerten), Improve (Daten ergaenzen) fuehrt zurÃ¼ck zu Evaluate — ein iterativer Kreislauf zur Verbesserung der Test-Daten" />

Dataset Management ist ein Zyklus: Du sammelst Seed-Daten, fuehrst Evals aus, bewertest die QualitÃ¤t Deiner Test-Daten, ergaenzt fehlende Faelle und evaluierst erneut. Die Daten werden mit jedem Durchlauf besser.
...
**Mit guten Datasets:** 20-50 diverse Test-Cases decken Normalfaelle, Edge Cases und Grenzfaelle ab. Wenn der Score 0.85 sagt, weisst Du, dass er repraesentativ ist. Du erkennst SchwÃ¤chen frueh und kannst gezielt verbessern.
...
evalite('Chat Titles', {
  data: async () => {
    ...
    return [
      ...
      {
        input: 'Wie konfiguriere ich ESLint fÃ¼r ein React-Projekt?',
...
| Phase | Anzahl | Zweck |
|-------|--------|-------|
| **Prototyp** | 5-10 | Schnelles Feedback, grundlegende Funktion prÃ¼fen |
...
data: async () => [
  ...
  { input: 'Was ist O(n log n)?', expected: 'Algorithmus-KomplexitÃ¤t' },
...
Definiere Kategorien fÃ¼r Dein Dataset und stelle sicher, dass jede abgedeckt ist:

```typescript
// Dataset-Kategorien fÃ¼r einen Chat-Title-Generator
const categories = {
  technical: [
...
Eine fortgeschrittene Technik: Lass ein LLM die QualitÃ¤t Deiner Test-Daten bewerten. "Sind meine Test-Cases repraesentativ genug? Welche Faelle fehlen?"
...
Das Ergebnis sagt Dir, welche Kategorien fehlen und schlÃ¤gt konkrete Test-Cases vor. Du pruefst die Vorschlaege, uebernimmst die sinnvollen und fuehrst die Evals erneut aus.

### Schicht 5: Dataset versionieren

Dein Dataset Ã¤ndert sich Ã¼ber die Zeit. Versioniere es wie Code:
...
So kannst Du spÃ¤ter vergleichen: Hat sich der Score mit Dataset v2 verÃ¤ndert?

## TRY

**Aufgabe:** Erstelle ein Dataset fÃ¼r einen Chat-Title-Generator mit mindestens 15 Test-Cases aus verschiedenen Kategorien.
...
const chatTitleDataset = [
  ...
  { input: 'Ich verstehe das nicht, kannst du es nochmal erklÃ¤ren?', expected: 'ErklÃ¤rung wiederholen' },
...
**ErklÃ¤rung:** Die Levenshtein-Scores werden niedrig sein, weil die simulierte `task` nur die ersten 5 Woerter nimmt — nicht den idealen Titel. Das ist der Punkt: Mit dem Dataset siehst Du, WO die Task versagt, und kannst iterativ verbessern. Fehlende Kategorien koennten sein: Code-Snippets als Input, URLs, Fragen in Nicht-Lateinischen Schriften.

</details>

## COMBINE

<Image src={combineDiagram} alt="Dataset (6.4) fliesst durch task() zu Output, der durch Deterministic (6.2) und Factuality (6.3) bewertet wird und im Dashboard endet" />
