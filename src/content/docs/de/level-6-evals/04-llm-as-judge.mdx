...
Der Factuality Scorer nutzt eine 5-stufige Skala von A bis E. **Achtung:** Das sind keine Schulnoten! Im deutschen Schulsystem waere A = 1 = sehr gut. Hier beschreiben die Buchstaben verschiedene *Verhaeltnisse* zwischen Antwort und Referenz — A ist nicht die beste Note, C (Identical) hat den hoechsten Score:
...
| **E** | Irrelevant Diff | 1.0 | Antworten unterscheiden sich, aber die Unterschiede sind fuer die Faktizitaet nicht relevant |
...
Wir nutzen `gpt-4o` als Judge-Modell — es eignet sich besonders gut fuer nuancierte inhaltliche Bewertungen. Der Scorer nutzt `generateObject` aus dem AI SDK, um eine strukturierte Bewertung vom Judge-LLM zu bekommen:
