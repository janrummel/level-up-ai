...
```mermaid
graph LR
  File[".eval.ts Datei"] --> Data["data<br/>(Input + Expected)"]
  Data --> Task["task<br/>(LLM Call)"]
  Task --> Scorers["scorers<br/>(Bewertung)"]
  Scorers --> Dashboard["Score Dashboard<br/>localhost:3006"]
  subgraph HIER["Du bist HIER: evalite()"]
    Data
    Task
    Scorers
  end
  ...
  style Scorers fill:#27AE60,color:#fff
...
```mermaid
graph LR
  Prompt["prompt"] --> GT["generateText()<br/>(Level 1.3)"]
  System["system"] --> GT
  GT --> Output["result.text"]
  Output --> Scorer["Levenshtein<br/>Scorer"]
  ...
  style Scorer fill:#27AE60,color:#fff
