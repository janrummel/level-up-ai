...
description: "Richte Evalite ein und schreibe Deine erste automatisierte Eval fÃ¼r LLM-Ausgaben — mit Daten, Task und Scorer."
sidebar:
  order: 2
---
...
Wie testest Du, ob Dein LLM-System gute Antworten gibt — manuell jedes Mal durchlesen? Was, wenn Du 50 Prompts aenderst und prÃ¼fen willst, ob die QualitÃ¤t gestiegen oder gesunken ist?
...
**Ohne Evals:** Du aenderst einen Prompt und hoffst, dass die Antworten besser werden. Du liest manuell 5 Antworten durch, die sehen gut aus, also deployest Du. Zwei Wochen spÃ¤ter merkst Du: Bei einer bestimmten Frage halluziniert das LLM jetzt. Keine Ahnung, seit wann.
...
`evalite watch` startet den Watch-Modus — wie `vitest --watch`. Bei jeder Aenderung an `.eval.ts` Dateien werden die Evals automatisch neu ausgefuehrt.
...
4. Ergebnisse werden gespeichert (standardmaessig in-memory; fÃ¼r persistente Speicherung konfigurierst Du SQLite via `createSqliteStorage` in einer `evalite.config.ts`)
5. Dashboard zeigt Scores unter `http://localhost:3006`

### Schicht 4: data als async Function

Statt eines statischen Arrays kann `data` auch eine async Function sein — nuetzlich fÃ¼r dynamisches Laden:
...
evalite('Capital Cities', {
  ...
  task: async (input) => {
    // Hier kommt spÃ¤ter Dein LLM-Call rein
    return input;
  },
  scorers: [Levenshtein],
});
...
- **Ãbersicht:** Alle Evals mit Durchschnitts-Score
- **Detail-Ansicht:** Jeder Test-Case mit Input, Output, Expected und Score
- **Verlauf:** Scores Ã¼ber die Zeit — siehst Du Verbesserungen oder Regressionen
...
- [ ] `task` gibt den Input mit angehaengtem Text zurÃ¼ck
...
**ErklÃ¤rung:** Der Levenshtein Scorer misst die Aehnlichkeit zwischen dem tatsaechlichen Output und dem erwarteten Output. Score 1.0 bedeutet identisch, Score 0.0 bedeutet komplett verschieden. In diesem Beispiel sollte jeder Test-Case einen Score von 1.0 erreichen, weil die `task`-Funktion exakt den erwarteten Output produziert.
...
5. Starte `pnpm eval:dev` und prÃ¼fe die Scores im Dashboard
