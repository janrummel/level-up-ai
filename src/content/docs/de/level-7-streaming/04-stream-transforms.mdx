...
description: "Glaette stotternde LLM-Streams mit smoothStream und baue eigene TransformStreams fÃ¼r Custom-Verarbeitung."
sidebar:
  order: 4
---
...
1. **Aeussere Funktion:** Gibt die Factory zurÃ¼ck (Konfigurationsebene)
2. **Innere Funktion:** Bekommt Options (Tools etc.), gibt `TransformStream` zurÃ¼ck
...
**Achtung:** Stream Transforms arbeiten chunk-weise. Eine E-Mail-Adresse kÃ¶nnte Ã¼ber zwei Chunks verteilt sein ("user@" + "example.com"). FÃ¼r robuste Filterung brauchst Du einen Buffer im Transform — das ist eine fortgeschrittene Technik.
...
1. `smoothStream()` fÃ¼r fluessige Ausgabe nutzt
