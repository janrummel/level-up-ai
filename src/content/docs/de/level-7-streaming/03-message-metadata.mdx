...
Wie haengst Du zusaetzliche Infos an eine Message an, die NICHT zum LLM gehen sollen — z.B. einen Timestamp, die User-ID oder die IP-Adresse? Alles in den Message-Content zu packen wÃ¤re Verschwendung von Tokens und kÃ¶nnte das LLM verwirren.

## OVERVIEW

import l7C2Overview from '../../../../assets/diagrams/l7-c2-overview.svg';
import l7C2Combine from '../../../../assets/diagrams/l7-c2-combine.svg';
import { Image } from 'astro:assets';

<Image src={l7C2Overview} alt="Overview: Message Content geht zum LLM Provider, Metadata geht zur Datenbank/Analytics — Routing über die API Route" />

Message Metadata lebt neben dem Content, wird aber NICHT an das LLM gesendet. Der Content geht zum LLM, die Metadata bleibt in Deiner App — fÃ¼r Logging, Persistence, Analytics.

## WHY

**Ohne Metadata:** Du packst alles in den Message-Content. Das LLM sieht irrelevante Informationen ("userId: abc123, timestamp: 1709..."), verschwendet Tokens und wird mÃ¶glicherweise verwirrt. Oder Du speicherst Metadaten in einer separaten Datenstruktur und musst sie manuell mit den Messages synchron halten — fehleranfaellig und umstaendlich.
...
Die `metadata`-Property wird vom AI SDK beim Senden an den LLM-Provider automatisch ignoriert. Sie ist ausschliesslich fÃ¼r Deine Anwendungslogik.
...
Du kannst auch Metadata an die Antwort-Messages haengen. Das passiert Ã¼ber `messageMetadata` in `toUIMessageStreamResponse`:
...
Im Frontend ist die Metadata dann auf der Assistant-Message verfÃ¼gbar:
...
### Schicht 4: Metadata fÃ¼r Persistence
...
// TODO 3: Sende an streamText und prÃ¼fe ob das LLM die Metadata erwaehnt
// const result = streamText({
//   model: anthropic('claude-sonnet-4-5-20250514'),
//   messages,
// });

// TODO 4: Konsumiere den Stream und prÃ¼fe die Antwort
...
> Das LLM sollte **nur** den Text-Content erwaehnen — keine userId, kein timestamp, keine sessionId. Falls es Metadata erwaehnt, prÃ¼fe ob `metadata` korrekt als Eigenschaft (nicht im `content`) steht.

</details>

## COMBINE

<Image src={l7C2Combine} alt="Combine: User Message mit Metadata wird über API Route zu Persistence und streamText() geleitet, Antwort mit Response Metadata ans Frontend" />
