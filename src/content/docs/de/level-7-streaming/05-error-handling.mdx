...
Was passiert in Deiner App, wenn der LLM-Provider mitten im Stream einen Fehler wirft? Der User hat schon die halbe Antwort gelesen — und dann? Weisse Seite? Abbruch ohne ErklÃ¤rung? Oder eine hilfreiche Fehlermeldung?
...
**Mit Error Handling:** Fehler werden abgefangen und in verstaendliche Meldungen uebersetzt. Der User weiss, was passiert ist, und kann es nochmal versuchen. Dein Logging erfasst den Fehler fÃ¼r Debugging. Und mit Retry-Strategien kann Deine App bestimmte Fehler sogar selbst loesen.
...
Wichtig: `onError` verhindert nicht, dass der Fehler zum User durchschlaegt. Es ist ein Hook fÃ¼r Logging und Monitoring — nicht fÃ¼r User-Facing Error Messages.
...
FÃ¼r Web-APIs kontrollierst Du mit `onError` in `toUIMessageStreamResponse`, welche Fehlermeldung der Client sieht:
...
export async function POST(req: Request) {
  ...
  return result.toUIMessageStreamResponse({
    onError(error) {                                  // ← Kontrolliert Client-Fehlermeldung
      // WICHTIG: Gibt den String zurÃ¼ck, den der Client sieht
...
- **`onError` in `streamText`**: Server-Seite. Logge den vollen Fehler fÃ¼r Debugging.
- **`onError` in `toUIMessageStreamResponse`**: Client-Seite. Gib eine kurze, verstaendliche Meldung zurÃ¼ck.
...
FÃ¼r transiente Fehler (Netzwerk, Rate Limits) kannst Du einen Retry-Wrapper bauen:
...
> **Hinweis:** Dieses Pattern wartet auf den vollstaendigen Text (`result.text`). FÃ¼r Streaming-Retries (wo Du den Text schon waehrend der Generierung anzeigen willst) brauchst Du eine komplexere LÃ¶sung — z.B. den `for await`-Loop in der Retry-Schleife selbst.

### Schicht 5: Try/Catch um den Stream-Consumer

Vergiss nicht: Fehler kÃ¶nnen auch beim Konsumieren des Streams auftreten. Wrap den `for await` Loop in einen try/catch:
...
try {
  ...
  console.log('Die Antwort konnte nicht vollstÃ¤ndig geladen werden.');
}
```
...
**ErklÃ¤rung:** Der ungueltige Modellname fuehrt zu einem API-Fehler. `onError` loggt den Fehler serverseitig (mit allen Details fÃ¼r Debugging). Der try/catch um den Stream-Consumer faengt den Fehler ab und zeigt eine verstaendliche Meldung. In Production wuerdest Du hier eine UI-Komponente rendern statt `console.log`.
