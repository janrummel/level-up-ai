...
Was passiert in Deiner App, wenn der LLM-Provider mitten im Stream einen Fehler wirft? Der User hat schon die halbe Antwort gelesen — und dann? Weisse Seite? Abbruch ohne Erklaerung? Oder eine hilfreiche Fehlermeldung?

## OVERVIEW

```mermaid
graph TD
  Start["streamText()"] --> Stream{"Stream laeuft"}

  Stream -->|Erfolg| Finish["finish Event<br/>Alles gut"]
  Stream -->|Fehler| Error["Error Event"]

  subgraph HIER["Du bist HIER: Error Handling"]
    Error --> OnError["onError Callback"]
    OnError --> UserMsg["User-freundliche<br/>Fehlermeldung"]
    OnError --> Log["Error Logging"]
    OnError --> Retry["Retry?"]
  end

  Retry -->|Ja| Start
  Retry -->|Nein| Fallback["Fallback-Antwort"]
...
**Mit Error Handling:** Fehler werden abgefangen und in verstaendliche Meldungen uebersetzt. Der User weiss, was passiert ist, und kann es nochmal versuchen. Dein Logging erfasst den Fehler fuer Debugging. Und mit Retry-Strategien kann Deine App bestimmte Fehler sogar selbst loesen.
...
Wichtig: `onError` verhindert nicht, dass der Fehler zum User durchschlaegt. Es ist ein Hook fuer Logging und Monitoring — nicht fuer User-Facing Error Messages.
...
Fuer Web-APIs kontrollierst Du mit `onError` in `toUIMessageStreamResponse`, welche Fehlermeldung der Client sieht:
...
- **`onError` in `streamText`**: Server-Seite. Logge den vollen Fehler fuer Debugging.
- **`onError` in `toUIMessageStreamResponse`**: Client-Seite. Gib eine kurze, verstaendliche Meldung zurueck.
...
Fuer transiente Fehler (Netzwerk, Rate Limits) kannst Du einen Retry-Wrapper bauen:
...
> **Hinweis:** Dieses Pattern wartet auf den vollstaendigen Text (`result.text`). Fuer Streaming-Retries (wo Du den Text schon waehrend der Generierung anzeigen willst) brauchst Du eine komplexere Loesung — z.B. den `for await`-Loop in der Retry-Schleife selbst.

### Schicht 5: Try/Catch um den Stream-Consumer

Vergiss nicht: Fehler koennen auch beim Konsumieren des Streams auftreten. Wrap den `for await` Loop in einen try/catch:
...
**Erklaerung:** Der ungueltige Modellname fuehrt zu einem API-Fehler. `onError` loggt den Fehler serverseitig (mit allen Details fuer Debugging). Der try/catch um den Stream-Consumer faengt den Fehler ab und zeigt eine verstaendliche Meldung. In Production wuerdest Du hier eine UI-Komponente rendern statt `console.log`.
...
graph LR
  ...
  Transform --> ErrorCheck{"Fehler?"}
  ErrorCheck -->|Nein| UI["Frontend"]
  ErrorCheck -->|Ja| Handler["onError<br/>(7.4)"]
  Handler --> UserMsg["User-Meldung"]
  Handler --> Retry["Retry mit<br/>Backoff"]
