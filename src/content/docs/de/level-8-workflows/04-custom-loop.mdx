...
**Ohne Custom Loop:** Du nutzt `generateText` mit `maxSteps` oder `stopWhen: stepCountIs(N)` (aus Level 3). Das reicht fuer Standard-Faelle. Aber: Du kannst nur nach Step-Count abbrechen, keinen eigenen State zwischen Iterationen tracken und keine Entscheidungen basierend auf Tool-Ergebnissen treffen.
