...
description: "Verhindere Endlosschleifen in Agent-Loops — Max-Iterations, Timeout, Cost Guard und Quality Check fÃ¼r kontrollierten Abbruch."
sidebar:
  order: 5
---
...
const searchTool = tool({
  ...
  execute: async ({ query }) => ({
    results: [`Ergebnisse fÃ¼r "${query}"...`],
  }),
});

const messages: Array<any> = [
  { role: 'user', content: 'Recherchiere Edge Computing ausfÃ¼hrlich.' },
];
...
FÃ¼r zeitkritische Anwendungen — der Loop muss innerhalb einer bestimmten Zeit fertig werden:
...
// AbortController fÃ¼r sauberen Abbruch
...
try {
  while (!done) {
    // Timeout prÃ¼fen vor jedem Call
...
while (!done) {
  ...
  // Cost Guard: Token-Budget prÃ¼fen
...
async function robustAgentLoop(prompt: string) {
  ...
  try {
    while (true) {
      ...
      // Pre-Check: Limits prÃ¼fen BEVOR der nÃ¤chste Call startet
...
// Ausfuehren
const output = await robustAgentLoop('Recherchiere Edge Computing ausfÃ¼hrlich.');
...
Die Funktion gibt immer ein Ergebnis zurÃ¼ck — inklusive `breakReason`. Der Caller weiss, ob der Agent fertig wurde (`complete`) oder ob ein Limit gegriffen hat. Das ist wichtig fÃ¼r Monitoring und Debugging.
...
//   - Max Iterations prÃ¼fen
//   - Timeout prÃ¼fen
//   - abortSignal an generateText uebergeben
//   - try/catch fÃ¼r AbortError
//   - bestResult speichern

// TODO 5: Gib das Ergebnis mit breakReason und Statistik zurÃ¼ck
```
...
const output = await guardedLoop(
  'Recherchiere Edge Computing. Nutze search fÃ¼r Recherche, dann save um das Ergebnis zu speichern.',
);
...
**ErklÃ¤rung:** Zwei Safeguards schuetzen den Loop: Max Iterations (pre-check vor jedem Call) und Timeout (via `AbortController` + manueller Check). Der Loop gibt immer ein Ergebnis zurÃ¼ck — auch bei Abbruch. Der `breakReason` zeigt an, ob der Agent fertig wurde oder ein Limit gegriffen hat.
...
Der Agent soll recherchieren, die QualitÃ¤t prÃ¼fen und abbrechen, sobald eine der fuenf Bedingungen erfuellt ist. Das Ergebnis enthaelt immer `breakReason` und `stats`.
