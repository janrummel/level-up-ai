...
**Erklaerung:** `createDataStream` oeffnet einen gemischten Kanal. `writeData` sendet Custom Data Parts (JSON-Zeilen, Prefix `2:`) vor und nach jedem Step. `streamText` + `mergeIntoDataStream` fuegt den Text-Stream des letzten Steps ein (Prefix `0:`). Der User sieht: Fortschritts-Updates fuer Step 1 und 2, dann den gestreamten Text von Step 3 — alles in Echtzeit.
