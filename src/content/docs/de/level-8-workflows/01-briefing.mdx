...
<CardGrid>
  <LinkCard title="8.1 — Workflow" description="Sequentielle `generateText`-Calls, Output-Verkettung, spezialisierte Steps" href="/de/level-8-workflows/02-workflow/" />
  <LinkCard title="8.2 — Streaming to Frontend" description="`createDataStream` + `writeData` fuer Step-Fortschritt, Frontend-Konsum" href="/de/level-8-workflows/03-streaming-to-frontend/" />
  <LinkCard title="8.3 — Custom Loop" description="Eigener while-Loop, Messages-Array, State-Tracking, finishReason-Check" href="/de/level-8-workflows/04-custom-loop/" />
  <LinkCard title="8.4 — Breaking the Loop" description="Max-Iterations, Timeout, Cost Guard, Quality Check, Partial Results" href="/de/level-8-workflows/05-breaking-the-loop/" />
</CardGrid>
