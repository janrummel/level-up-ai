...
- **Custom Loop** — eigene Agent-Loops bauen mit voller Kontrolle über Abbruchbedingungen, State-Tracking und Messages-Array
...
> **Projektverzeichnis:** Erstelle einen neuen Ordner `level-8-workflows/` für die Dateien dieses Levels. Du brauchst keine neuen Pakete — `ai`, `@ai-sdk/anthropic` und `zod` aus vorherigen Levels genuegen.
...
<CardGrid>
  ...
  <LinkCard title="8.2 — Streaming to Frontend" description="`createDataStream` + `writeData` für Step-Fortschritt, Frontend-Konsum" href="../03-streaming-to-frontend/" />
