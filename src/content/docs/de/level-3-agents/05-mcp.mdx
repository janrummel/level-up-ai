...
graph LR
  ...
  Protocol --> Server1["MCP Server:<br/>Dateien"]
  Protocol --> Server2["MCP Server:<br/>Datenbank"]
  Protocol --> Server3["MCP Server:<br/>Web API"]
  Server1 --> FS["Filesystem"]
  Server2 --> DB["PostgreSQL"]
  Server3 --> API["REST API"]

  subgraph HIER["Du bist HIER: MCP"]
    Client
    Protocol
  end
...
graph LR
  ...
  Loop -->|Lokal| Local["Lokales Tool<br/>(3.1)"]
  MCP --> Server["MCP Server"]
  Server --> MCP
  MCP --> Loop
  Local --> Loop
  Loop -->|Fertig| Text["Antwort"]
