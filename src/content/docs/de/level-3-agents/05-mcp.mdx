...
description: "Verbinde Dein LLM mit externen Diensten Ã¼ber ein standardisiertes Protokoll — MCP Client, Transport Types und Tool-Integration."
sidebar:
  order: 5
---
...
Wie verbindest Du Dein LLM mit externen Diensten — ohne fÃ¼r jeden eine eigene Integration zu bauen? Was wÃ¤re, wenn es ein standardisiertes Protokoll gaebe, das jeder Tool-Server spricht?

## OVERVIEW

<Image src={overviewDiagram} alt="AI App verbindet sich Ã¼ber MCP Client und Protocol mit drei MCP Servern fÃ¼r Dateien, Datenbank und Web API" />
...
**Ohne MCP:** FÃ¼r jeden externen Dienst baust Du eine eigene Integration. Eigene `tool()`-Definition, eigene API-Anbindung, eigene Fehlerbehandlung. Bei 10 Diensten hast Du 10 verschiedene Integrationen zu pflegen. Nicht standardisiert, nicht austauschbar.

**Mit MCP:** Ein Protokoll fÃ¼r alle Tools. Du verbindest Dich mit einem MCP Server und bekommst automatisch alle seine Tools — mit Beschreibungen, Schemas, fertig zur Nutzung. Server wechseln? Transport Ã¤ndern? Ein Konfigurationswechsel, kein Code-Umbau.

## WALKTHROUGH

### Setup fÃ¼r diese Challenge
...
MCP unterstuetzt verschiedene Transports fÃ¼r verschiedene Szenarien:
...
**HTTP** — fÃ¼r Remote-Server in Production:
...
**stdio** — fÃ¼r lokale Server waehrend der Entwicklung:
...
stdio startet den MCP Server als lokalen Kindprozess. Kommunikation laeuft Ã¼ber stdin/stdout. Ideal fÃ¼r Development und Testen.

### Schicht 3: MCP Tools laden und nutzen

`client.tools()` gibt Dir alle Tools des Servers als Objekt zurÃ¼ck — fertig fÃ¼r `generateText`:
...
const tools = await client.tools();                      // ← Gibt { toolName: tool, ... } zurÃ¼ck
...
`client.tools()` gibt ein Objekt zurÃ¼ck, das genau das Format hat, das `generateText` erwartet. Du kannst MCP Tools und lokale Tools auch mischen:
...
Standardmaessig sind MCP Tools untypisiert — die Schemas kommen vom Server. Du kannst aber eigene Zod-Schemas angeben fÃ¼r volle TypeScript-Typsicherheit:
...
**Aufgabe:** Erstelle einen MCP Client mit stdio Transport, lade die verfÃ¼gbaren Tools und nutze sie mit `generateText`.
...
**ErklÃ¤rung:** Der `@modelcontextprotocol/server-everything` ist ein offizieller Demo-Server, der verschiedene Test-Tools bereitstellt (echo, add, etc.). Der stdio Transport startet ihn als lokalen Kindprozess. `client.tools()` laedt automatisch alle verfÃ¼gbaren Tools mit ihren Beschreibungen und Schemas. Nach der Nutzung wird der Client — und damit der Kindprozess — sauber geschlossen.
