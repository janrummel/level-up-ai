...
description: "Verbinde Dein LLM mit externen Diensten ueber ein standardisiertes Protokoll — MCP Client, Transport Types und Tool-Integration."
sidebar:
  order: 5
---
...
Wie verbindest Du Dein LLM mit externen Diensten — ohne fuer jeden eine eigene Integration zu bauen? Was waere, wenn es ein standardisiertes Protokoll gaebe, das jeder Tool-Server spricht?

## OVERVIEW

<Image src={overviewDiagram} alt="AI App verbindet sich ueber MCP Client und Protocol mit drei MCP Servern fuer Dateien, Datenbank und Web API" />
...
**Ohne MCP:** Fuer jeden externen Dienst baust Du eine eigene Integration. Eigene `tool()`-Definition, eigene API-Anbindung, eigene Fehlerbehandlung. Bei 10 Diensten hast Du 10 verschiedene Integrationen zu pflegen. Nicht standardisiert, nicht austauschbar.

**Mit MCP:** Ein Protokoll fuer alle Tools. Du verbindest Dich mit einem MCP Server und bekommst automatisch alle seine Tools — mit Beschreibungen, Schemas, fertig zur Nutzung. Server wechseln? Transport aendern? Ein Konfigurationswechsel, kein Code-Umbau.
...
MCP unterstuetzt verschiedene Transports fuer verschiedene Szenarien:
...
**HTTP** — fuer Remote-Server in Production:
...
**stdio** — fuer lokale Server waehrend der Entwicklung:
...
stdio startet den MCP Server als lokalen Kindprozess. Kommunikation laeuft ueber stdin/stdout. Ideal fuer Development und Testen.

### Schicht 3: MCP Tools laden und nutzen

`client.tools()` gibt Dir alle Tools des Servers als Objekt zurueck — fertig fuer `generateText`:
...
const tools = await client.tools();                      // ← Gibt { toolName: tool, ... } zurueck
...
`client.tools()` gibt ein Objekt zurueck, das genau das Format hat, das `generateText` erwartet. Du kannst MCP Tools und lokale Tools auch mischen:
...
Standardmaessig sind MCP Tools untypisiert — die Schemas kommen vom Server. Du kannst aber eigene Zod-Schemas angeben fuer volle TypeScript-Typsicherheit:
...
**Aufgabe:** Erstelle einen MCP Client mit stdio Transport, lade die verfuegbaren Tools und nutze sie mit `generateText`.
...
**Erklaerung:** Der `@modelcontextprotocol/server-everything` ist ein offizieller Demo-Server, der verschiedene Test-Tools bereitstellt (echo, add, etc.). Der stdio Transport startet ihn als lokalen Kindprozess. `client.tools()` laedt automatisch alle verfuegbaren Tools mit ihren Beschreibungen und Schemas. Nach der Nutzung wird der Client — und damit der Kindprozess — sauber geschlossen.
