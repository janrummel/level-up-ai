...
description: "Human-in-the-Loop fÃ¼r kritische Operationen — mit needsApproval, dynamischer Approval-Logik und sicheren Agents."
sidebar:
  order: 6
---
...
Wuerdest Du einem Agenten erlauben, ohne Rueckfrage Dateien zu loeschen, E-Mails zu versenden oder Daten in einer Datenbank zu Ã¤ndern?

## OVERVIEW

<Image src={overviewDiagram} alt="LLM will Tool ausfÃ¼hren, Approval Guard fragt User, bei Genehmigung wird Tool ausgefuehrt, bei Ablehnung abgelehnt" />

Zwischen Tool Call und Tool Execution sitzt ein **Approval Guard**. Wenn ein Tool als kritisch markiert ist, fragt der Guard den User um Erlaubnis, bevor das Tool ausgefuehrt wird. Das ist Human-in-the-Loop — der Mensch behaelt die Kontrolle Ã¼ber gefaehrliche Aktionen.
...
**Mit Approval:** Kritische Operationen werden erst nach menschlicher Freigabe ausgefuehrt. Du definierst, welche Tools sicher sind (automatisch ausfÃ¼hren) und welche gefaehrlich sind (erst fragen). Der Agent bleibt nuetzlich, aber kontrollierbar.
...
const deleteFileTool = tool({
  ...
  needsApproval: async ({ path }) => {                   // ← Async Funktion
    // Nur Freigabe noetig fÃ¼r kritische Verzeichnisse
...
Die `needsApproval`-Funktion bekommt die gleichen Parameter wie `execute`. Sie gibt `true` zurÃ¼ck, wenn eine Freigabe noetig ist, und `false`, wenn das Tool direkt ausgefuehrt werden darf.
...
// Antwort-Messages in den Verlauf Ã¼bernehmen
  ...
  for (const part of result.content) {
    if (part.type === 'tool-approval-request') {
      const approved = await askUser(
        `Agent will ${part.toolCall.toolName}(${JSON.stringify(part.toolCall.input)}) ausfÃ¼hren. Erlauben? (y/n): `
      );
      approvals.push({
...
1. **Erster Aufruf:** `generateText` erkennt, dass `deleteFile` eine Freigabe braucht. Statt das Tool auszufuehren, kommt ein `tool-approval-request` in `result.content` zurÃ¼ck.
2. **Approval Request prÃ¼fen:** Jeder Request hat eine `approvalId` und den `toolCall` (mit `toolName` und `input`).
...
// Sichere Tools — automatisch ausfÃ¼hren
...
// TODO 6: While-Loop: result.content auf 'tool-approval-request' prÃ¼fen
// - FÃ¼r jeden Request: User fragen, ToolApprovalResponse bauen
...
**ErklÃ¤rung:** Der Agent liest zuerst `/tmp/notes.txt` (automatisch, kein Approval noetig) und versucht dann die Datei zu loeschen. Beim `deleteFile` Call greift `needsApproval: true`: Statt das Tool auszufuehren, gibt `generateText` einen `tool-approval-request` in `result.content` zurÃ¼ck. Dein Code fragt den User im Terminal. Bei "y" wird eine genehmigende `ToolApprovalResponse` gebaut und als Tool-Message angehaengt. Der zweite `generateText`-Aufruf fuehrt das Tool dann aus. Bei "n" bekommt das LLM die Ablehnung und reagiert darauf — z.B. mit "Die Datei wurde nicht geloescht, da die Aktion abgelehnt wurde."
...
<Image src={combineDiagram} alt="Prompt startet Agent, LLM waehlt sichere oder gefaehrliche Tools, gefaehrliche brauchen Approval, bei Ablehnung zurÃ¼ck zum LLM" />

**Uebung:** Erweitere den Research Agent aus Challenge 3.3 um einen Approval-Schritt fÃ¼r das Speichern von Ergebnissen.
...
4. Implementiere den 2-Call Loop: Erster `generateText`-Aufruf, dann `result.content` auf `tool-approval-request` prÃ¼fen, User fragen, `ToolApprovalResponse` bauen, zweiter Aufruf
