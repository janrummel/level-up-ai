...
import overviewDiagram from '../../../../assets/diagrams/l3-c2-overview.svg';
...
<Image src={overviewDiagram} alt="LLM sendet tool-call und tool-result Events an UI, UI zeigt Loading State und Ergebnis-Karte" />
...
Wenn Du `streamText` mit Tools nutzt, bekommst Du Ã¼ber `fullStream` spezifische Events:
...
Eine vollstÃ¤ndige Rendering-Logik muss alle Part-Typen behandeln. Hier ein Beispiel, das Text, Tool Calls und Tool Results formatiert ausgibt:
...
for await (const part of result.fullStream) {
  switch (part.type) {
    ...
    case 'tool-call':
      ...
      console.log(`│  Status: ausfÃ¼hrend...`);
      break;
    case 'tool-result':
...
Die `Map` trackt den Status jedes Tool Calls Ã¼ber seine `toolCallId`. In einem echten Frontend wÃ¼rde das ein React State sein, der die UI aktualisiert.
...
// TODO 2: Iteriere Ã¼ber result.fullStream
...
**ErklÃ¤rung:** Das LLM ruft `weather` fÃ¼r "Berlin" und "Muenchen" auf — mÃ¶glicherweise in separaten Tool Calls. Jeder Call erzeugt erst ein `tool-call` Event (mit Args), dann nach Ausfuehrung ein `tool-result` Event (mit Ergebnis). Am Ende kommt Text, der beide Ergebnisse zusammenfasst.
...
<Image src={combineDiagram} alt="User Prompt und System Prompt fliessen in streamText, fullStream liefert text-delta, tool-call, tool-result und finish Events" />
...
**Optional Stretch Goal:** Baue einen `renderToolResult(toolName, result)` Helper, der fÃ¼r verschiedene Tools unterschiedliche Formatierungen ausgibt — z.B. eine Wetter-Anzeige fÃ¼r `weather` und eine Berechnung fÃ¼r `calculator`.
