...
const calculatorTool = tool({
  ...
  inputSchema: z.object({
    expression: z.string().describe('The math expression, e.g. "2024 - 2010"'),
  }),
  execute: async ({ expression }) => ({
    expression,
    result: eval(expression),                            // ← Nur fuer Demo! Nie eval in Production.
  }),
});
...
Das LLM koennte so vorgehen: Schritt 1 → `search("Rust Geschichte")` → Schritt 2 → `calculator("2026 - 2010")` → Schritt 3 → `summarize(...)` → Schritt 4 → Finale Antwort. Die Reihenfolge entscheidet das LLM autonom.
