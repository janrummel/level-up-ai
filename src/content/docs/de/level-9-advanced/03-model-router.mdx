...
description: "Waehle automatisch das richtige Modell fÃ¼r jede Aufgabe — guenstig fÃ¼r Einfaches, stark fÃ¼r Komplexes, spezialisiert fÃ¼r Code."
sidebar:
  order: 3
---
...
Wuerdest Du fÃ¼r die Frage "Wie spaet ist es?" dasselbe Modell nutzen wie fÃ¼r "Analysiere diesen 500-Zeilen Code und finde den Performance-Bug"? Was kostet es, wenn Du immer das teuerste Modell nimmst?
...
**Ohne Model Routing:** Du nutzt ein teures Modell fÃ¼r alles. Einfache Fragen wie "Was ist TypeScript?" kosten genauso viel wie komplexe Analysen. Bei 10.000 Requests pro Tag summieren sich die Kosten schnell. Oder Du nutzt ein billiges Modell fÃ¼r alles — dann leidet die QualitÃ¤t bei komplexen Aufgaben.

**Mit Model Routing:** Jede Aufgabe bekommt das passende Modell. 80% der Anfragen sind einfach und gehen an das Flash-Modell (Faktor 10-50x guenstiger). 20% sind komplex und bekommen das Pro-Modell. Ergebnis: gleiche QualitÃ¤t, drastisch niedrigere Kosten.
...
function selectModel(task: 'simple' | 'complex' | 'code') {
  switch (task) {
    ...
    case 'code':
      return anthropic('claude-sonnet-4-5-20250514');   // ← Gutes Preis-Leistungs-Verhaeltnis fÃ¼r Code
  }
}
...
function estimateTokens(text: string): number {
  return Math.ceil(text.split(/\s+/).length * 1.3);    // ← Grobe SchÃ¤tzung: Woerter * 1.3
}
...
Token-basiertes Routing ist ein guter erster Schritt — lange Prompts deuten oft auf komplexe Aufgaben hin. Aber Laenge allein ist kein perfekter Proxy fÃ¼r KomplexitÃ¤t.
...
async function classifyTask(input: string) {
  const result = await generateText({
    model: google('gemini-2.5-flash-lite'),             // ← Kleines Modell fÃ¼r Klassifikation
...
- "code": Code schreiben, debuggen, reviewen, erklÃ¤ren`,
    prompt: input,
...
const answer = await routedGenerate(
  'Vergleiche die Vor- und Nachteile von REST vs. GraphQL fÃ¼r eine Microservices-Architektur.',
);
console.log(answer);
```

Zwei LLM-Calls statt einem — aber der erste (Klassifikation) ist extrem guenstig und schnell. Die Kosten fÃ¼r die Klassifikation sind vernachlaessigbar im Vergleich zur Ersparnis, wenn 80% der Anfragen ans Flash-Modell gehen.

### Schicht 4: Kostenvergleich

Was bringt Model Routing konkret? Eine Ãbersicht der Modellkosten:

:::note[Preise Stand: Maerz 2025]
Modellpreise Ã¤ndern sich regelmÃ¤Ãig — prÃ¼fe die aktuellen Preise auf den jeweiligen Provider-Seiten ([Anthropic](https://docs.anthropic.com/en/docs/about-claude/models), [Google AI](https://ai.google.dev/gemini-api/docs/models)) bevor Du Kosten kalkulierst.
:::

| Modell | Input (pro 1M Tokens) | Output (pro 1M Tokens) | StÃ¤rke |
|--------|----------------------|------------------------|---------|
...
Das ist eine Ersparnis von Ã¼ber 90% — bei vergleichbarer QualitÃ¤t, weil die einfachen Anfragen kein Opus brauchen.
...
//   - Nutze Output.enum fÃ¼r typsichere Ausgabe
...
- [ ] `Output.enum` fÃ¼r typsichere Klassifikation genutzt
...
- "code": Code schreiben, debuggen, erklÃ¤ren`,
    prompt: input,
...
const testInputs = [
  ...
  'Vergleiche SQL vs. NoSQL Datenbanken fÃ¼r eine E-Commerce-Plattform',
...
**ErklÃ¤rung:** Zwei LLM-Calls pro Anfrage — der erste (Klassifikation) kostet nahezu nichts mit Flash Lite. Der zweite Call geht an das passende Modell. Die `Output.enum`-Funktion aus Level 1.5 garantiert, dass die Klassifikation immer einer der drei erlaubten Werte ist.
...
--- Input: "Vergleiche SQL vs. NoSQL Datenbanken fÃ¼r eine E-Commerce-Plattform" ---
