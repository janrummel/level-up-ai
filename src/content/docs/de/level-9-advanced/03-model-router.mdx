...
```mermaid
graph TD
  Input["User Input"] --> Analyze{"Input<br/>analysieren"}
  Analyze -->|Einfach| Flash["Flash-Modell<br/>(guenstig)"]
  Analyze -->|Komplex| Pro["Pro-Modell<br/>(teuer)"]
  Analyze -->|Code| Code["Code-Modell<br/>(spezialisiert)"]
  Flash --> Output["Output"]
  Pro --> Output
  Code --> Output
...
graph LR
  ...
  Router --> Model["Gewahltes<br/>Modell"]
  Model --> Result["Result"]
