...
Die folgenden Keyword-basierten Checks sind ein **didaktischer Startpunkt**, um das Guardrail-Pattern zu verstehen. In Production nutzt Du ML-basierte Classifier oder spezialisierte APIs (z.B. OpenAI Moderation API, Anthropics eingebaute Safety-Layer) fuer robustere Erkennung. Keyword-Matching ist leicht zu umgehen.
...
Beachte: Einfache String-Matching-Guardrails koennen false positives erzeugen — z.B. wuerde `'hack'` auch `'hackathon'` blockieren, und `'exploit'` auch `'Wir exploiten diese Luecke nicht'`. In Production brauchst Du differenziertere Ansaetze wie Wortgrenzen-Matching (`\bhack\b`), Tokenisierung oder ML-basierte Classifier.
:::
...
Output-Guardrails pruefen drei Dimensionen: Laenge (nicht zu kurz, nicht zu lang), Format (valides JSON wenn erwartet) und Meta-Refusals (erkennen wenn das LLM sich weigert oder seine AI-Natur referenziert).
