...
graph LR
  Topic["Topic"] --> R
  subgraph HIER["Du bist HIER: Research Workflow"]
...
Diese Pipeline verbindet Konzepte aus dem gesamten Lernpfad. Hier ist die Uebersicht, welches Level wo zum Einsatz kommt:

| Phase | Konzept | Level | Funktion |
|-------|---------|-------|----------|
| Research | Tool Calling | 3.1 | `search`-Tool fuer Datensammlung |
...
| Processing | Context Engineering | 5.x | XML-strukturierte Prompts fuer praezise Outputs |
| Processing | Structured Output | 1.5 | Zod Schema fuer typisierte Metadaten |
| Quality | Guardrails | 9.1 | Input/Output-Validierung |
| Quality | Model Router | 9.2 | Optimales Modell pro Phase |
| Quality | Usage Tracking | 2.2 | Token-Kosten ueber die gesamte Pipeline |
...
Wenn Du genug Informationen hast, fasse die Ergebnisse zusammen.`,
  ...
  try {
    while (true) {
      ...
      const result = await generateText({
        model: google('gemini-2.5-flash'),              // ← Guenstiges Modell fuer Research
  ...
  // Letzten Text-Output extrahieren
...
- Wenn Informationen fehlen, sage es explizit
  ...
  return {
    report: structured.experimental_output,
...
Die Quality-Phase ist die letzte Verteidigungslinie. Guardrails pruefen den finalen Output auf Laenge, Inhalt und Qualitaet. Die gesamte Pipeline trackt Tokens und Dauer — wichtig fuer Production Monitoring.
...
// TODO 1: Definiere ein search-Tool (simuliert, gibt festen Text zurueck)
...
**Erklaerung:** Drei Phasen — Research mit Tool-Loop (max 3 Iterationen), Summarize mit eigenem System Prompt, Format mit Guardrails. Token-Verbrauch wird ueber alle Phasen kumuliert. Die Pipeline laeuft von Anfang bis Ende durch, auch wenn der Research-Loop frueh abbricht.
...
- **Level 6:** Evals — Qualitaet messen statt raten
