---
title: "Level 9 Complete — Lernpfad abgeschlossen"
description: "Du hast alle 9 Levels gemeistert. Vom Vibe Coder zum AI Engineer — der komplette Skill Tree, Abschluss und naechste Schritte."
sidebar:
  order: 7
---

## Was Du gelernt hast

- **Guardrails:** Input- und Output-Checks die Prompt Injection, PII-Leaks und unkontrollierte Ausgaben verhindern. Das Middleware-Pattern macht Guardrails composable und wiederverwendbar. Doppelte Absicherung: Code-Guardrails fangen technisch erkennbare Probleme ab, Prompt-Guardrails geben dem LLM Verhaltensregeln.
- **Model Router:** Automatisch das richtige Modell fuer jede Aufgabe waehlen — von einfacher Switch-Logik ueber Token-basiertes Routing bis zu LLM-basierter Klassifikation. Das Ergebnis: gleiche Qualitaet bei bis zu 90% niedrigeren Kosten.
- **Comparing Outputs:** Mehrere Modelle parallel befragen mit `Promise.all`, Ergebnisse mit einfachen Metriken und LLM-as-a-Judge vergleichen, datengetriebene Modell-Entscheidungen treffen statt auf Bauchgefuehl zu vertrauen.
- **Research Workflow:** Eine End-to-End Pipeline die Konzepte aus allen 9 Levels vereint — Agent Loop mit Tools, sequentielle Workflows, Context Engineering, Structured Output, Guardrails, Model Routing und Usage Tracking. Das grosse Bild.

## Der komplette Skill Tree

```mermaid
graph LR
  subgraph "Freigeschaltet — Alle 9 Levels"
    L1["Level 1: AI SDK Basics"]
    L2["Level 2: LLM Fundamentals"]
    L3["Level 3: Agents & MCP"]
    L4["Level 4: Persistence"]
    L5["Level 5: Context Engineering"]
    L6["Level 6: Evals"]
    L7["Level 7: Streaming"]
    L8["Level 8: Workflows"]
    L9["Level 9: Advanced Patterns"]
  end
  L1 --> L2
  L2 --> L3
  L3 --> L4
  L4 --> L5
  L5 --> L6
  L6 --> L7
  L7 --> L8
  L8 --> L9
  style L1 fill:#27AE60,color:#fff
  style L2 fill:#27AE60,color:#fff
  style L3 fill:#27AE60,color:#fff
  style L4 fill:#27AE60,color:#fff
  style L5 fill:#27AE60,color:#fff
  style L6 fill:#27AE60,color:#fff
  style L7 fill:#27AE60,color:#fff
  style L8 fill:#27AE60,color:#fff
  style L9 fill:#27AE60,color:#fff
```

Alle 9 Levels freigeschaltet. 37 Challenges gemeistert. 9 Boss Fights bestanden.

## Du bist kein Vibe Coder mehr

Du hast als jemand angefangen, der ChatGPT nutzt und AI-Code per Copy-Paste einbaut. Jetzt bist Du ein **AI Engineer** — jemand der versteht, wie AI-Systeme von innen funktionieren und sie gezielt, sicher und kosteneffizient baut.

Das ist der Unterschied:

| Vibe Coder | AI Engineer |
|------------|-------------|
| Kopiert Code aus ChatGPT | Baut Systeme mit dem AI SDK |
| Ein Modell fuer alles | Routet zum optimalen Modell |
| "Funktioniert irgendwie" | Evals beweisen die Qualitaet |
| Keine Ahnung von Kosten | Token-Tracking und Kostenoptimierung |
| Copy-Paste Prompts | Context Engineering mit XML-Struktur |
| Keine Sicherheit | Input/Output Guardrails |
| Einzelne API-Calls | Orchestrierte Workflows und Agent Loops |
| Hofft auf gute Antworten | Vergleicht Modelle systematisch |

**Vom Vibe Coder zum AI Engineer.** Du hast den Lernpfad abgeschlossen.

## Was Du jetzt kannst

Hier ist, was Du mit dem Wissen aus allen 9 Levels bauen kannst:

- **AI-gesteuerte Produkte** — Chatbots, Research-Assistenten, Content-Pipelines, Code-Review-Tools
- **Production-Ready Systeme** — mit Guardrails, Error Handling, Cost Management, Eval Coverage
- **Multi-Modell Architekturen** — Model Routing, Comparing Outputs, Provider-agnostische Systeme
- **Autonome Agents** — Tool Calling, Custom Loops, Break Conditions, Multi-Step Workflows

## Naechste Schritte

Kein naechstes Level — aber der Lernpfad endet nicht hier. Hier sind die besten Ressourcen um weiterzumachen:

### Offizielle Dokumentation

- **[ai-sdk.dev](https://ai-sdk.dev)** — Die offizielle AI SDK Dokumentation. Deine erste Anlaufstelle fuer neue Features, API-Referenzen und Guides.
- **[docs.anthropic.com](https://docs.anthropic.com)** — Anthropic's Dokumentation fuer Claude. Prompt Engineering, Best Practices, API-Referenz.
- **[platform.openai.com](https://platform.openai.com/docs)** — OpenAI's Plattform-Dokumentation. Models, API, Best Practices.
- **[ai.google.dev](https://ai.google.dev)** — Google AI Dokumentation fuer Gemini. Models, API, Tutorials.

### Tools und Frameworks

- **[Evalite](https://www.evalite.dev)** — Das Eval-Framework das Du in Level 6 gelernt hast. Fuer systematische Qualitaetssicherung.
- **[Zod](https://zod.dev)** — Schema Validation fuer TypeScript. Die Basis fuer Structured Output und Tool Parameters.
- **[MCP (Model Context Protocol)](https://modelcontextprotocol.io)** — Der Standard fuer Tool-Integration, den Du in Level 3 kennengelernt hast.

### Eigene Projekte bauen

Das Wichtigste: **Bau etwas.** Nimm ein Problem das Du hast und loese es mit dem was Du gelernt hast. Ideen:

1. **Personal Research Assistant** — Ein CLI-Tool das ein Thema recherchiert, zusammenfasst und als Markdown-Report speichert. Nutze die Research Pipeline aus Level 9.
2. **Code Review Bot** — Ein Agent der Pull Requests analysiert, Probleme findet und Verbesserungen vorschlaegt. Nutze Tools (Level 3), Context Engineering (Level 5) und Guardrails (Level 9).
3. **Multi-Model Chat** — Ein Chat der Antworten von mehreren Modellen vergleicht und dem User die beste zeigt. Nutze Comparing Outputs (Level 9), Streaming (Level 7) und Persistence (Level 4).
4. **Content Pipeline** — Ein System das Blog-Posts generiert: Research, Outline, Draft, Edit, Format. Nutze Workflows (Level 8) mit Evals (Level 6) fuer Qualitaetssicherung.

### Community

- **[GitHub: Vercel AI SDK](https://github.com/vercel/ai)** — Issues, Discussions, Contributions
- **[GitHub: ai-hero-dev](https://github.com/ai-hero-dev)** — Die Exercises auf denen dieser Lernpfad aufbaut

## Danke

Dieser Lernpfad ist Open Source und kostenlos. Wenn er Dir geholfen hat, teile ihn mit anderen die den Weg vom Vibe Coder zum AI Engineer gehen wollen.
