...
Du baust ein **Production-Ready AI-System** — das ultimative Projekt, das alles vereint was Du in 9 Levels gelernt hast. Das System nimmt ein Thema entgegen, recherchiert autonom, waehlt fÃ¼r jeden Schritt das optimale Modell, schuetzt sich mit Guardrails, und liefert einen qualitaetsgesicherten Report.
...
### Pflicht — das muss Dein System kÃ¶nnen
...
2. **Model Router** (Challenge 9.2) — Nutze fÃ¼r jede Phase der Pipeline das optimale Modell. Research: guenstiges Flash-Modell. Analyse und Formatierung: starkes Sonnet/Opus-Modell. Klassifikation: kleinstes verfuegbares Modell.
...
### Bonus — fÃ¼r den Extra-Kick

8. **Comparing Outputs** (Challenge 9.3) — FÃ¼r den finalen Report: Generiere die Zusammenfassung mit 2 verschiedenen Modellen parallel und waehle das bessere Ergebnis (per einfachem Scorer oder LLM-as-a-Judge).

9. **Streaming** (Level 7 + 8.2) — Streame den finalen Report in Echtzeit. Sende Progress Data Parts fÃ¼r jede Phase der Pipeline. (Nutze `createDataStream` + `writeData` wie in Challenge 8.2.)

10. **Eval Coverage** (Level 6) — Schreibe mindestens 2 Evalite-Tests: einen der prÃ¼ft ob der Report die erwarteten Abschnitte hat, und einen der die Laenge der Zusammenfassung bewertet.
...
- [ ] Input-Guardrails prÃ¼fen auf Prompt Injection und PII
- [ ] Model Router waehlt unterschiedliche Modelle fÃ¼r Research vs. Analysis
...
- [ ] Streaming mit Progress Data Parts fÃ¼r jede Pipeline-Phase
- [ ] Mindestens ein Evalite-Test prÃ¼ft die Pipeline-QualitÃ¤t
...
Baue die Pipeline als eine `async function researchPipeline(topic: string)` die alle drei Phasen nacheinander ausfÃ¼hrt. Jede Phase ist eine eigene Funktion die ein Ergebnis-Objekt zurueckgibt. Am Ende: Statistiken sammeln und den Report zurueckgeben.
...
Schreibe die Evals in eine separate Datei. Importiere die Pipeline-Funktion und teste sie mit Evalite. Ein Scorer prÃ¼ft ob `keyFindings.length >= 3`, ein anderer ob `summary.length > 100`. So bleiben Pipeline und Tests sauber getrennt.
