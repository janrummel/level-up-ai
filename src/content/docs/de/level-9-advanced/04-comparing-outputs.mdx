...
graph LR
  ...
  Prompt --> B["Model B"]
  Prompt --> C["Model C"]
  subgraph HIER["Du bist HIER: Comparing Outputs"]
    A --> Judge{"Judge LLM"}
    B --> Judge
    C --> Judge
  end
...
Vergib Scores von 1-10 und begruende Deine Bewertung.`,
    ...
    experimental_output: Output.object({ schema: JudgmentSchema }),
  });

  return judgment.experimental_output;
}
...
graph LR
  ...
  RB --> Eval
  RC --> Eval
  Eval --> Report["Vergleichs-<br/>Report"]
  style Prompt fill:#4A90D9,color:#fff
