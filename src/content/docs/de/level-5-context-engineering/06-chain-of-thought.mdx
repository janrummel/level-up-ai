...
description: "Bringe das LLM dazu, Schritt für Schritt nachzudenken — für nachvollziehbare, fehleraermere Antworten."
sidebar:
  order: 6
---
...
<Image src={overviewDiagram} alt="Links: direkte Frage ans LLM ergibt fehleranfaellige Antwort (rot). Rechts (Chain of Thought): Frage geht ins LLM das Schritt für Schritt denkt, durch thinking-instructions und output-format gefuehrt, und eine nachvollziehbare Antwort liefert" />
...
```
Ohne CoT:  "Was ist 37 * 83?"  →  "2981"  (geraten, falsch — korrekt wäre 3071)
Mit CoT:   "Was ist 37 * 83?"  →  "37 * 80 = 2960, 37 * 3 = 111, 2960 + 111 = 3071"  →  "3071"
```

Die Tokens, die das LLM beim Nachdenken generiert, beeinflussen die Qualität der eigentlichen Antwort. Der Plan steht im Kontext — und der Kontext steuert, was als naechstes kommt.
...
2. Ich muss erst Grundtypen erklären
...
CoT ist nicht für jede Aufgabe sinnvoll. Es kostet zusaetzliche Tokens und verlangsamt die Antwort.
...
//   - Abhängigkeiten zwischen Fixes beachten
...
**Erklärung:** Die `<thinking-instructions>` fuehren das LLM durch einen systematischen Analyseprozess: erst lesen, dann Probleme identifizieren, dann priorisieren. Das `<output-format>` trennt den Denkprozess (`<thinking>` Block) von den strukturierten Review-Ergebnissen. So kann man nachvollziehen, warum das LLM welches Issue wie bewertet hat — und die Ergebnisse sind konsistent formatiert.

**Erwarteter Output (ungefaehr):**
```
<thinking>
1. any[] — kein Typsicherheit, Laufzeitfehler möglich
2. == statt === — lose Vergleiche können zu Bugs fuehren
...
**Uebung:** Baue einen vollstaendigen Prompt der ALLE 5 Konzepte dieses Levels vereint: Template (5.1) + XML Struktur (5.2) + Exemplars (5.3) + RAG (5.4) + Chain of Thought (5.5). Das ist die Vorbereitung für die Boss Fight.
