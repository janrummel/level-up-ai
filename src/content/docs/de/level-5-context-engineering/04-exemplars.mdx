...
Beachte: Gute Exemplars koennen `<rules>` und `<output-format>` ergaenzen und in manchen Faellen sogar effektiver sein. Die Beispiele transportieren Format, Laenge und Stil implizit — fuer komplexe Logik oder Randfaelle sind explizite Rules aber weiterhin sinnvoll.
