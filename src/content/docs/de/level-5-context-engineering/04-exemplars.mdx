...
import overviewDiagram from '../../../../assets/diagrams/l5-c3-overview.svg';
import combineDiagram from '../../../../assets/diagrams/l5-c3-combine.svg';

## THINK

Wie wuerdest Du einem neuen Mitarbeiter einen bestimmten Schreibstil erklÃ¤ren — mit Regeln oder mit Beispielen?

## OVERVIEW

<Image src={overviewDiagram} alt="Neuer Input fliesst in einen Prompt mit task-context, dem hervorgehobenen examples-Tag (input/output Paare) und the-ask, der dann ins LLM geht und konsistenten Output im gewuenschten Stil erzeugt" />
...
**Ohne Exemplars:** Das LLM interpretiert Stil und Format frei. Du schreibst ausfÃ¼hrliche Regeln ("Titel sollen maximal 30 Zeichen haben, in Title Case, ohne Punkt am Ende"), aber das LLM haelt sich nicht immer daran. Die Ausgaben variieren.
...
Das Prinzip ist einfach: Wenn Du einem neuen Mitarbeiter zeigst, wie drei fertige E-Mails aussehen, versteht er den gewuenschten Stil sofort — ohne 20 Regeln lesen zu mÃ¼ssen. Genauso funktioniert es mit LLMs.
...
- Den Stil (keine Satzzeichen, keine ErklÃ¤rung)

### Schicht 3: Wie viele Exemplars braucht man?

Die Faustregel: **2-5 Exemplars sind optimal.**

- **1 Exemplar:** Reicht fÃ¼r einfache Formate, aber das LLM kÃ¶nnte es als Einzelfall interpretieren
- **2-3 Exemplars:** Gut fÃ¼r die meisten Faelle — das Muster wird klar
- **4-5 Exemplars:** Ideal fÃ¼r komplexe Formate oder wenn Randfaelle abgedeckt werden mÃ¼ssen
...
Beachte: Gute Exemplars kÃ¶nnen `<rules>` und `<output-format>` ergaenzen und in manchen Faellen sogar effektiver sein. Die Beispiele transportieren Format, Laenge und Stil implizit — fÃ¼r komplexe Logik oder Randfaelle sind explizite Rules aber weiterhin sinnvoll.
...
Erstelle **`challenge-5-3.ts`** und fuehre aus mit: **`npx tsx challenge-5-3.ts`**
...
const exemplars = [
  {
    input: 'Das neue Update ist fantastisch! Endlich funktioniert alles schnell und zuverlÃ¤ssig.',
    output: 'positiv',
  },
  {
    input: 'Der Kundenservice war eine Katastrophe. Drei Stunden Wartezeit und keine LÃ¶sung.',
    output: 'negativ',
  },
  {
...
**ErklÃ¤rung:** Die drei Exemplars decken alle drei Kategorien ab. Das LLM erkennt aus den Beispielen, dass die Ausgabe genau ein Wort sein soll (`positiv`, `negativ` oder `neutral`). Der Text "okay, nichts Besonderes aber auch nicht schlecht" wird korrekt als `neutral` klassifiziert — ohne dass wir explizite Regeln dafuer definieren mussten.

</details>

## COMBINE

<Image src={combineDiagram} alt="PromptConfig fliesst in buildSystemPrompt, Exemplars Array in ein Prompt Template mit examples-Slot, beide fliessen in generateText und erzeugen konsistenten Output" />
