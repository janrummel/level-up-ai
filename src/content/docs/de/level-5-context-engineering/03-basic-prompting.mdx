...
```mermaid
graph LR
  subgraph "Ohne Struktur"
    VP["Vager Prompt<br/>'Fasse zusammen'"] --> LLM1["LLM"]
    LLM1 --> BAD["Unvorhersehbar<br/>Mal Liste, mal Absatz"]
  end
  subgraph HIER["Du bist HIER: Basic Prompting"]
    subgraph "Mit XML Tags"
      TC2["&lt;task-context&gt;"] --> SP["Strukturierter<br/>Prompt"]
      R2["&lt;rules&gt;"] --> SP
      TA2["&lt;the-ask&gt;"] --> SP
      OF2["&lt;output-format&gt;"] --> SP
      SP --> LLM2["LLM"]
      LLM2 --> GOOD["Konsistent<br/>Reproduzierbar"]
    end
  end
...
graph LR
  ...
  Input["User Input"] --> Prompt["Strukturierter Prompt<br/>mit XML Tags"]
