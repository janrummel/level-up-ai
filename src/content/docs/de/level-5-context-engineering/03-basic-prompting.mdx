...
**Ohne Struktur:** Jedes Mal eine andere QualitÃ¤t. Das LLM weiss nicht, welches Format Du willst, wie lang die Antwort sein soll oder welche Rolle es hat. Mal bekommst Du eine Liste mit 10 Punkten, mal einen Absatz, mal eine Frage zurÃ¼ck. Keine Kontrolle, keine Reproduzierbarkeit.
...
Warum diese Reihenfolge? LLMs gewichten den Anfang und das Ende eines Prompts stÃ¤rker als die Mitte. Kritische Anweisungen gehÃ¶ren deshalb an die Raender.

### Schicht 2: Jedes Tag erklÃ¤rt mit Beispiel
...
// Schlecht: Das LLM weiss nicht, was fÃ¼r ein Titel, wie lang, welches Format
...
const result = await generateText({
  ...
  prompt: `
<task-context>
Du bist ein technischer Redakteur, der Artikel fÃ¼r Entwickler zusammenfasst.
</task-context>
...
<output-format>
Gib nur die Zusammenfassung zurÃ¼ck. Keine Ueberschrift, keine Einleitung,
keine zusaetzliche ErklÃ¤rung.
</output-format>
  `.trim(),
});
...
**ErklÃ¤rung:** Der Prompt definiert eine klare Rolle (`technischer Redakteur`), stellt den Artikel als `<background-data>` bereit, setzt konkrete Regeln (3 Saetze, Deutsch, sachlich) und kontrolliert das Ausgabeformat. Das LLM weiss genau, was erwartet wird.
...
1. Nutze `buildSystemPrompt()` aus Challenge 5.1 fÃ¼r den `system`-Parameter
